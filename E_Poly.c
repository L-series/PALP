...
void DYNComplete_Poly(Long VPM[][VERT_Nmax], EqList *_E, int nv, DYN_PPL *_CP) {
  ...
  Long yDen[POLY_Dmax];
  int OrdFac[VERT_Nmax], BasFac[POLY_Dmax], one[POLY_Dmax], position[POLY_Dmax];
  ...
  if (n == 0) {
    ...
    for (i = 0; i < _P_AUX->n; i++) {
      for (j = 0; j < _P_AUX->np; j++)
        fprintf(outFILE, (_P_AUX->np > 20) ? " %3d" : " %4d",
                (int)_P_AUX->x[j][i]);
      fprintf(outFILE, "\n");
...
int Interval_Check(int rank, Poset_Element *_x, Poset_Element *_y, Cone *_C) {
  int flag = 0;
...
int Make_Mirror(EPoly *_EP, int h[][POLY_Dmax], int D, int dim) {
  ...
  for (u = 0; u < 4 * (Pos_Max); u++)
    for (v = 0; v < 3 * (Pos_Max); v++) {
      if (((-2 * D + u) > dim) || ((-2 * D + u) < 0) || ((-2 * D + v) > dim) ||
          ((-2 * D + v) < 0)) {
        if (_EP->E[u][v] != 0)
  ...
  if (H != 0)
    Die("paper: CY 4-folds and toric fibrations; equation (8) don't hold");
  if (dim == 4) {
...
void Output(PolyPointList *_P,                        /* PolyPointList * _DP,*/
            ...
            VertexNumList *_V, int *_n, PartList *_PTL, int *_codim,
            FILE *outFILE, Flags *_F, int *_D, LInfo *_L) {
  int i, j, k, m, chi = 0, D = (_P_D->n + 1), dim = (_P->n - *_codim);
  int h[POLY_Dmax][POLY_Dmax] = {{0}, {0}}, S[VERT_Nmax];

  m = Max_S(_PTL, _n);
  if (((!_PTL->DirProduct[*_n]) || _F->Dir) &&
      ((!_PTL->Proj[*_n]) || _F->Proj)) {
    if (_F->H == 0) {
      if (_F->w) {
        PrintWeights(_W);
        fprintf(outFILE, " ");
...
void Make_S_Poly(Cone *_C, VertexNumList *_V, EqList *_E, PolyPointList *_P,
                 Poset_Element_List *_PEL, SPoly *_S, int SINFO,
                 int CHECK_SERRE) {
        ...
        for (u = 0; u < (dim + 1); u++)
          for (v = 0; v <= (dim / 2); v++)
            _EP->E[2 * _C->dim - x + y + dmax - u - *_codim]
                ...
                _S_D[_IL->L[i].min].S[x] *
                _S_N[_PEL->n - _IL->L[i].max - 1].S[y] * _BL[i].B[u][v] * s;
  }
}
...
void Make_Cone(Cone *_C_D, Cone *_C_N, FaceInfo *_I, PolyPointList *_P) {
  ...
  _C_D->nface[_P->n + 1] = 1;
  _C_N->nface[_P->n + 1] = 1;
  _C_D->edge[0][0] = INCI_0();
  _C_N->edge[0][0] = INCI_0();
  _C_D->edge[_C_D->dim][0] = INCI_0();
  for (i = 0; i < _I->nf[0]; i++)
    _C_D->edge[_C_D->dim][0] = INCI_OR(_C_D->edge[_C_D->dim][0], _I->v[0][i]);
...
void Make_E_Poly(FILE *outFILE, CWS *_W, PolyPointList *_CP, VertexNumList *_CV,
                 EqList *_CE, int *_codim, Flags *_F, int *_D) {
  time_t Tstart;
  clock_t Cstart;
  int n;
  /* Interval_List IL;
  ...
  fflush(0);
  {
    ...
    if (_F->y && (_PTL->n != 0) && (_W->nw == 0))
      fprintf(outFILE, "%d %d Vertices of Poly in M-lattice:  ", _DP->n,
              _DV->nv);
    if (!(_F->y && (_PTL->n == 0))) {
  ...
  for (n = 0; n < _PTL->n; n++) {
    ...
    if (((!_PTL->Proj[n]) || _F->Proj) && ((!_PTL->DirProduct[n]) || _F->Dir)) {
      if (_F->g)
        PRINT_GORE(_P_D, *_codim, _F->gd, "Points of PG:");
      if (_F->d)
        PRINT_GORE(_P_N, *_codim, _F->dd, "Points of dual PG:");
    }
  }
  if (!_F->n && !_F->g && !_F->d && !_F->y)
    Print_Nefinfo(_PTL, /* _F,*/
                  &Tstart, &Cstart);
  if (_F->n && N_Part(_PTL))
    PRINT_PL(_P, "Points of Poly in N-Lattice:");
...
void AnalyseGorensteinCone(CWS *_CW, PolyPointList *_P, VertexNumList *_V,
  ...
  if (r) {
    ...
    for (i = 0; i < _V->nv; i++) {
      for (k = 0; k < _V->nv; k++) {
        for (j = 0; j < _V_D->nv; j++)
          if (VPM[j][i] != VPM_D[k][j])
            break;
        if (j == _V_D->nv) {
          _new_E_D->e[i] = _E_D->e[k];
          break;
        }
      }
      if (k >= _V->nv) {
    ...
    if (!_F->g && !_F->d) {
      Compute_E_Poly(&EP, _P_D, _V_D, _new_E_D, _P, _V, _E, _codim, _F, &Tstart,
                     &Cstart);
    ...
    if (_F->g)
      Print_PPL(_P_D, "Points  of support in N:");
    if (_F->d)
    ...
    if (_F->N)
      fprintf(outFILE, "N:%d %d ", _P->np, _V->nv);
    else
      fprintf(outFILE, "M:%d %d ", _P->np, _V->nv);
