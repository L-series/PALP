...
int Read_WZ_PP(Weight *WZ) /* read "d w_i" [ or "w_i d" if last=max ] */
{
  ...
  *nz = 0;
  ...
  while (i < n) {
    ...
    for (k = 0; k < WZ->N; k++) {
      if ((j = auxString2SInt(&c[i], &WZ->z[*nz][k]))) {
        if (j)
          i += j;
        else
          WZerror("missing");
  ...
  if (1 == WZ->r)
    ...
    for (j = 0; j < WZ->M; j++) {
      ...
      for (n = 0; n < a; n++)
        s += WZ->z[j][n];
      if (s % WZ->m[j]) {
        puts("det=1 required if Gorenstein index r>1:");
#if DET_WARN_ONLY
        WZ->M = 0;
      }
#else
  ...
  for (i = 0; i < a; i++)
    for (j = 0; j < d; j++)
      WZ->B[i][j] = B[j + 1][i]; /* LATTICE BASIS */
  for (i = a - 1; i > 0; i--) {
    X = 1 - WZ->r * WZ->A[i];
    for (j = i; j < d; j++)
      X -= WZ->rI[j] * WZ->B[i][j];
  ...
  for (i = 0; i < *nz; i++) /* divide by symmetries: */
  {
    ...
    for (j = 0; j < d; j++) {
      Za[j] = 0;
      for (k = 0; k < a; k++)
        Za[j] += WZ->z[i][k] * WZ->B[k][j];
      g = NNgcd(Za[j], g);
    }
    if (Tout) {
      printf("WZ->m[%d]=%d g=%ld  ", i, WZ->m[i], g);
      if (g == WZ->m[i])
        puts("");
    }
    if (g == WZ->m[i])
...

  {
    ...
    if (i < d) /* rI!=0 => make P0=0 */
    {
      ...
      for (i = 0; i < a; i++)
        for (j = 0; j < d; j++)
          WZ->A[i] += WZ->B[i][j] * P->x[0][j];
      for (i = 0; i < d; i++) {
        WZ->rI[i] -= WZ->r * P->x[0][i];
        P->x[0][i] = 0;
      }
    }
  ...
  X = 0;
  for (j = 0; j < a; j++)
    X += WZ->w[j] * WZ->A[j]; /* X=w.A */
  for (i = 0; i < WZ->M; i++) {
    Long A = 0, M = WZ->m[i], cx, cm, g; /* A=Z.A */
    for (j = 0; j < a; j++)
      A += WZ->z[i][j] * WZ->A[j];
    if (0 == (A %= M))
      continue;
...
void Make_Poly_Points(Weight *_W_in, PolyPointList *_PP) {
  ...
  _W = &Waux;
  {
    int i, n = _W->N, pi[AMBI_Dmax];
    Long M[AMBI_Dmax][AMBI_Dmax], *G[AMBI_Dmax];
    for (i = 0; i < n; i++)
      G[i] = M[i];
    Wperm_to_GLZ(_W->w, &n, G, pi);
    for (i = 0; i < n; i++)
...
void NormTriangularBasis(AmbiLatticeBasis *_B) {
  ...
  while (p--) {
    ...
    while (--pi > p) {
      ...
      /**/ if ((_B->x[pi][a] % _B->x[p][a]) < 0)
        r--;
      /**/ if (r)
        while (ai--)
          _B->x[pi][ai] -= r * _B->x[p][ai];
    }
  }
}
void OldWeightLatticeBasis(Weight *_w, AmbiLatticeBasis *_B) {
  ...
  *Vout = Fgcd(Vin[0], Vin[1]);
  ...
  for (j = 2; j < _w->N; j++)
    V[j] = 0;
  for (i = 2; i < _w->N; i++) {
    int g = REgcd(Vin, &i, Vout), gn = Fgcd(g, _w->w[i]);
    V = _B->x[i - 1];
    V[i] = g / gn;
    g = _w->w[i] / gn;
    for (j = 0; j < i; j++)
...
void WeightMakePoints(Weight *_W, AmbiPointList *_P) {
  ...
  while (i < _W->N) {
    if (X[i] > X_max[i])
      ...
      d_Rest[i - 1] = d_Rest[i] - X[i] * _W->w[i];
      X[--i] = 0;
      X_max[i] = d_Rest[i] / _W->w[i];
    } else {
      int j;
...
#ifdef TEST
  if (ipcount - 1)
    puts("*** Wrong IP-count: take any point as origin! ***");
#endif
  for (n = 0; n < _AP->np; n++) {
    int i, p = _B->n;
    while (p--) {
      int a = p + _B->N - _B->n;
      _PP->x[n][p] = _AP->x[n][a] - _AP->x[nIP][a];
      for (i = p + 1; i < _B->n; i++)
...
{
  UnitPoly(P);
  Add_Mono_2_Poly(N, -1, P);
}
void Remove_Zeros(PoCoLi *AB) {
  int i, s = 0;
  for (i = 0; i < AB->n; i++) /* s=shift: eliminatet zeros */
  {
    if (0 == AB->c[i])
      s++;
    else if (s) {
...
int BottomUpQuot(PoCoLi *N, PoCoLi *D, PoCoLi *Q, PoCoLi *R) /* Q*D = N-R */
{
  ...
  for (Npos = 0; Npos < N->n; Npos++) {
    if (N->e[Npos] <= e + dD)
      Add_Mono_2_Poly(N->e[Npos], N->c[Npos], R);
    else
      break;
  }
  ...
  while (0 == (R->c[0] % D->c[0])) {
    ...
    for (i = 0; i < D->n; i++)
      Add_Mono_2_Poly(e + D->e[i], -c * D->c[i], R); /* R-cQ */
    Remove_Zeros(R);
    if ((Npos == N->n) && (0 == R->n))
      return 1;                                            /* finished? */
...
int DoHodgeTest(VaHo *V) /*[holo] Poincare duality, Hodge duality, sum rule */
{
  int i, j, D = V->D, X;
  for (i = D / 2; i <= D; i++) {
    if (V->h[i][0] != V->h[D - i][0])
      return 0;
    for (j = 0; j <= D; j++)
      if (V->h[i][j] != V->h[D - i][D - j])
        return 0;
  }
  V->E = X = 0;
  for (i = 0; i <= D; i++)
...
int Count_b01(Weight *W) {
  int b = 0, c = 0, l, M[POLY_Dmax], I[POLY_Dmax], eq[W_Nmax], j, J = W->M,
      N = W->N, vac = 0;
  for (j = 0; j < J; j++)
    M[j] = W->m[j];
  for (l = 0; l < W->d; l++) /* Z_d==GSO */
  {
    c = Init_Multiloop(M, I, &j, &J);
    do {                          /* BEGIN generate group */
      int i, can = 1, sum = 0, k; /* can(didate) for b01-contribution */
      for (i = 0; can && (i < N); i++) {
        Rat th = rR(l * W->w[i], W->d); /* th=0/q -> eq[] */
        for (k = 0; k < J; k++) {
      ...
      if (sum && (sum != W->d))
        can = 0; /* i.e. charge=1? */
      for (k = 0; can && (k < J); k++) {
        int gph = 0; /* check inv. under G_k: gph(ase) */
        for (i = 0; i < N; i++) {
          if (eq[i])
            gph += W->z[k][i];
          gph %= M[k];
                  ...
                  VaHo *V) /* -->> V.D<=3 <<--  via index and trace */
{
  ...
  for (l = 0; l < W->d; l++) /* Z_d==GSO */
  {
    c = Init_Multiloop(W->m, I, &j, &ns);
    do { /* BEGIN generate group */
      nvar = 0;
      for (i = 0; i < N; i++) {
        expo = l * W->w[i] * R;
        for (k = 0; k < ns; k++)
          expo += I[k] * omega[k] * W->z[k][i];
        if (!(expo % kgV))
          nvar += mask[i];
      }
      woS[nvar]++;
  ...
  for (i = 0; i < mask[n]; i++) {
    ...
    if (woS[i])
      for (k = 0; k < mask[n]; k++)
        if (woS[k]) { /* woS = sum of contrib */
          wort = i & k;
          wo[fac + 1][wort] += woS[i] * woS[k];
        }
  }
  for (i = mask[n] - 1; i >= 0; i--)
    if (woG[i] + woA[i]) {
      prod = rI(1);
      for (j = 0; j < n; j++)
        if ((mask[j] & i) == mask[j])
...
void LGO_VaHo(Weight *W, VaHo *V) {
  ...
  {
    Long dQ, QL, q;
    PoCoLi *S, *SO, *SN; /* dQ=sum(1-2th)=QR-QL; QL=sum(th-w) */
    int j, k, s, J = W->M, M[POLY_Dmax], I[POLY_Dmax], X = W->m[0],
                 x[POLY_Dmax];
    int WI, T;
    Long ph[POLY_Dmax], th[W_Nmax], U = X * d / Fgcd(X, d), rd = U / d, G = 1;
    PoCoLi Den, Num, Quo, Rem;
    MobiusData MX;
    MakeMobius(&MX, X);
    Den.A = (1 << W->N);
    AllocPoCoLi(&Den);
...
int OLDTrans_Check(
  ...
  for (i = 0; i < W.N; i++)
    for (j = mask[i]; j < mask[i + 1]; j++) {
      targets[j] = 0;
      for (k = 0; k <= i; k++)
        targets[j] = targets[j] | targets[j & ~mask[k]];
      for (k = 0; (k < W.N) && (mighty[targets[j]] < mighty[j]); k++)
        if (!(mask[k] & targets[j]))
          if (OLDsymcheck(W.d - W.w[k], j, &mon, W, mask))
            targets[j] = targets[j] | mask[k];
      if (mighty[targets[j]] < mighty[j])
        return 0;
    }
  return 1;
}
  ...
  for (i = 0; i < W.N; i++)
    if (link & mask[i]) {
      ...
      for (j = 0; j * W.w[i] < sum.d; j++) {
        ...
        for (k = 0; k < W.M; k++)
          newsum.m[k] = sum.m[k] - j * W.z[k][i];
        if (symcheck(newsum, newlink, W, mask))
          return 1;
      }
    }
  return 0;
    ...
    for (j = mask[i]; j < mask[i + 1]; j++) {
      ...
      for (k = 0; (k < W.N) && (mighty[targets[j]] < mighty[j]); k++)
        if (!(mask[k] & targets[j])) { /* targets=lower bounds only! */
          ...
          for (l = 0; l < W.M; l++)
            dw.m[l] = W.m[l] - W.z[l][k]; /* gradient phases */
          if (symcheck(dw, j, W, mask))
            targets[j] = targets[j] | mask[k];
        }
      if (mighty[targets[j]] < mighty[j])
        return 0;
