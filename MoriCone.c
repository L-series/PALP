...
int Choose(int n, int k, int *C) {
  int x;
  for (x = 0; x < k - 1; x++)
    if (C[x] < C[x + 1] - 1) {
      int i; // hole => ++ and below -> min
      for (i = 0; i < x; i++)
        C[i] = i;
      C[x]++;
      return 1;
...
void DivClassBasis(int SF, PolyPointList *P, int v, char *D, char *B) {
  ...
  do {
    ...
    if (EXCEPT_DIV_CLASS_BASE)
      for (c = 0; c < d; c++)
        CC[c] = C[c];
    else
      for (c = 0; c < d; c++)
        CC[c] = v - 1 - C[c]; /* eliminate D_max first */
    for (c = 0; c < d; c++)
      X[c] = P->x[CC[c]];
...
void Inci64_2_VNL(Inci64 X, VertexNumList *V, int n) {
  int i;
  V->nv = 0;
  for (i = 0; i < n; i++)
    if (getN(i, X))
...
int Make_triCD2F(triang *T, Inci64 *cd2I) {
  ...
  for (i = 1; i < T->n; i++)
    for (j = 0; j < i; j++) {
      int k;
      Inci64 S = T->I[i] & T->I[j];
      for (k = 0; k < cd2n; k++)
        if (Inci64_LE(S, cd2I[k]))
          break;
        else if (Inci64_LE(cd2I[k], S))
          cd2I[k--] = cd2I[--cd2n];
      if (k == cd2n)
        cd2I[cd2n++] = S;
    }
  for (i = 0; i < cd2n; i++)
    if (Inci64_abs(cd2I[i]) != T->d - 1)
      break;
    ...
    for (j = 0; j < i; j++)
      if (Inci64_abs(Inci64_AND(I[i], I[j])) == d - 1) {
        ...
        if (Z[b] > 0)
          ...
          for (a = 0; a <= d; a++)
            R.x[r][V.v[a]] = -Z[a];
        // for(a=0;a<p;a++)printf("%ld
        // ",R.x[r][a]);printf("=Z::R[%d]\n",r);fflush(0);
        for (b = 0; b < r; b++)
          if (!Vdiff_LmR(R.x[r], R.x[b], p))
            break;
        if (b == r)
          r++;
      }
  R.v = UT->np = ngen = r;
  r = Make_G_for_GxMT_UT(R, G); // base change -> UT
  if (r != p - d) {
    Matrix GR;
    R.v = ngen;
    Print_LMatrix(R, "R");
...
void Print_Mori(PolyPointList *P, int p, int nI, Inci64 *I) {
  ...
  for (i = 0; i < E->ne; i++)
    if (Eval_Eq_on_V(&E->e[i], UT->x[np], UT->n) == 0) {
      e0++;
      for (j = 0; j < nv; j++)
...
void StanleyReisner(triang *SR,
  ...
  for (i = 1; i < p; i++)
    for (j = 0; j < i; j++) {
      M[m] = (U << i) + (U << j); /* OFFSET=0 */
      for (k = 0; k < nI; k++)
        if (Inci64_LE(M[m], I[k]))
          break; /* no primitive collect */
      if (k == nI)
        S[s++] = M[m];
      else
        assert(++m < binco);
  ...
  for (r = 3; r <= d + 1; r++) {
    ...
    for (k = 0; k < m; k++)
      for (x = 1 + MaxBit(M[k], p); x < p; x++) {
        N[n] = M[k] + (U << x);
        /*prnI(p,M[k]);printf("=M[%d] x=%d >
         * N[%d]=",k,x,n);prnI(p,N[n]);puts("");*/
        for (l = 0; l < nI; l++)
          if (Inci64_LE(N[n], I[l]))
            break;
        if (l == nI) {
          for (l = 0; l < s; l++)
            if (Inci64_LE(S[l], N[n]))
...
int Make_G_for_MxG_LT(Matrix M, Matrix *G) { /* MxG lower trian, return rank */
  Matrix MT;
  Init_Matrix(&MT, M.d, M.v);
  Transpose(M, MT);
  Init_Matrix(G, M.v, M.v);
  return Make_G_for_GxMT_UT(MT, *G);
}

void GaleTransform(Matrix A, Matrix *B) {
  Matrix G;
  int i, j;
  Make_G_for_MxG_LT(A, &G); // Print(A); Print(G);
...
int AcuteAngle(Long *L, Long *R) {
  return L[0] * R[0] + L[1] * R[1] > 0;
} // R.L>0
int ConeAngle(Long *L, Long *R) {
  Long X = L[1] * R[0] - L[0] * R[1]; // 0->0, (0,pi) ->+1
  if (X > 0)
    return 1;
  if (X < 0)
    return -1;
...
Long XYZproduct(Long *X, Long *Y, Long *Z) {
  return Z[2] * (X[0] * Y[1] - X[1] * Y[0]) +
         Z[0] * (X[1] * Y[2] - X[2] * Y[1]) +
         Z[1] * (X[2] * Y[0] - X[0] * Y[2]);
}
void CROSSproduct(Long *X, Long *Y, Long *XxY) {
  XxY[2] = X[0] * Y[1] - X[1] * Y[0];
  XxY[0] = X[1] * Y[2] - X[2] * Y[1];
  XxY[1] = X[2] * Y[0] - X[0] * Y[2];
}
Long SCALproduct(Long *X, Long *Y) {
...
int InterSectE(Long *X, Long *Y, Long *U, Long *V) {
  Long xyu = XYZproduct(X, Y, U), xyv = XYZproduct(X, Y, V),
       uvx = XYZproduct(U, V, X), uvy = XYZproduct(U, V, Y);
  if ((0 <= xyu * xyv) || (0 <= uvx * uvy))
    return 0;
  if (xyu * uvx < 0)
    return 1;
  else
...
void IntersectEdges(Long *X, Long *Y, Long *U, Long *V, Long *Q) {
  ...
  CROSSproduct(X, Y, XY);
  MakeVecPrim(XY);
  if (SCALproduct(XY, U) < 0)
    CROSSproduct(U, V, UV);
  else
    CROSSproduct(V, U, UV);
  MakeVecPrim(UV);
  CROSSproduct(XY, UV, Q);
  MakeVecPrim(Q);
                 ...
                 int *nt) // P,p,F[c],X,CT[nmf],&nmt[nmf],nt[nmf]
{
  int d = P->n, v = d + 2, i, j, k = 0, r, F[POLY_Dmax + 2], Z[POLY_Dmax + 2],
      z = 0, // GKZ polygon:
      R[VERT_Nmax][POLY_Dmax], nrp[VERT_Nmax], nr, neg = 0,
      tnt = 0; // num.max.tri
  Matrix A, B;
  Init_Matrix(&A, v, d);
  Inci64 U; // P.x::F[A]::F[Z[R]]
  for (i = 0; i < p; i++)
    if (getN(i, FI)) {
      for (j = 0; j < d; j++)
        A.x[k][j] = P->x[i][j];
      ...
      for (i = 1; i < nr; i++) {
        ...
        if (BZangle(*R[y], *R[x]) > 0)
          for (a = 0; a < nrp[x]; a++)
            for (b = 0; b < nrp[y]; b++)
              PC |= (CI[nt[*nmt]++] =
                         U - makeN(F[Z[R[x][a]]]) - makeN(F[Z[R[y][b]]]));
      }
    }
    if (PC == U) {
      tnt += nt[*nmt];
...
int ABCline(Long *a, Long *b, Long *c) {
  int i;                                   // 1:: edge(abc), -1:: acb|cab
  ...
  if (0 == (AC = SCALproduct(A, C)))
    return 0; // A/b^2, C/b^2 = ortho. proj.
  if (!(AA = SCALproduct(A, A)))
    return 0;
  if (!(CC = SCALproduct(C, C)))
    return 0;
  if (AA * CC != AC * AC)
    return 0;
  return (AC < 0) ? 1 : -1;
...
int XYZcone(Long *A, Long *B, Long *C) { //  1: B inside cone <AC>_+
  int i, j, k;
  Long x, y, z; // -1: <ABC>_+ is 2d strict conv. cone
  if (XYZproduct(A, B, C))
    return 0; //  0: <XYZ> 3d or non strictly convex
  for (k = 0; k < 3; k++) {
    i = (k + 1) % 3;
    j = (k + 2) % 3; //
    if ((x = B[i] * C[j] - B[j] * C[i]))
      break;
  } //  solve    x A + y B + z C = 0
  if (k == 3)
    return 0; //  coincident lines => return 0
  if (!(y = C[i] * A[j] - C[j] * A[i]))
    return 0;
  if (!(z = A[i] * B[j] - A[j] * B[i]))
    return 0;
  if (x * z > 0)
    return (x * y < 0) ? 1 : 0;
  else
...
int Triang3dSFan(PolyPointList *P, int p, Inci64 FI, Inci64 *X,
  ...
  Inci64 Einc[VERT_Nmax];
  Long CR[VERT_Nmax][3]; // chamber rays <
    ...
    for (b = 1; b < c; b++)
      for (a = 0; a < b; a++)
        if ((k = BZRx(a, b, c))) { // tn TRI
          ...
          for (k = 0; k < en; k++) {
            if (Einc[k] == (makeN(b) + makeN(c)))
              ea = 0; // en EDGES
            if (Einc[k] == (makeN(a) + makeN(c)))
              eb = 0;
            if (Einc[k] == (makeN(a) + makeN(b)))
              ec = 0;
          }
          if (ea) {
            Eli[en][0] = b;
            Eli[en][1] = c;
            Einc[en++] = makeN(b) + makeN(c);
          }
          if (eb) {
            Eli[en][0] = a;
            Eli[en][1] = c;
            Einc[en++] = makeN(a) + makeN(c);
          }
          if (ec) {
            Eli[en][0] = a;
            Eli[en][1] = b;
            Einc[en++] = makeN(a) + makeN(b);
          }
          tn++;
        }
  ...
  if (iem > 1) {
    ...
    for (f = r; f < y; f++) {
      int s = noe; // split edges
      // for(l=0;l<noe;l++)printf("oe%d=%d%d ",l,OE[l][0],OE[l][1]);puts("
      // split:");
      for (i = 0; i < noe; i++)
        if (0 < XYZcone(Y[OE[i][0]], Y[f], Y[OE[i][1]])) {
          OE[s][1] = OE[i][1];
          OE[s++][0] = OE[i][1] = f;
        }
      noe = s;
    ...
    for (i = 0; i < en; i++)
      if (ien[i])
        for (f = 1; f <= ien[i]; f++) {   // add split edge
          if ((j = IEli[i][f - 1]) > i) { // if new add Y[k]=Q to YY's
            IntersectEdges(BZRE(i, 0), BZRE(i, 1), BZRE(j, 0), BZRE(j, 1),
                           Y[y]);
            for (k = r; k < y; k++)
              if (SameRay(Y[k], Y[y], 3))
                break;
            if (k == y) {
              Qinc[q] = makeN(i) + makeN(j);
              Y[++y] = YY[++q];
              ...
              Qinc[k - r] |= makeN(i) + makeN(j);
          } else {
            for (k = 0; k < q; k++)
              if (Inci64_LE(makeN(i) + makeN(j), Qinc[k]))
                break;
            for (l = k + 1; l < q; l++)
              assert(!Inci64_LE(makeN(i) + makeN(j), Qinc[l]));
            assert(k < q);
            k += r;
          } // unique ray with Y[k]::E_i & E_j
          printf("#ie[%d]::i=%d/j=%d split at k=%d y=%d q=%d\n", f - 1, i, j, k,
                 y, q);
          for (l = 0; l <= noe + f; l++)
            printf("oe%d=%d%d ", l, OE[l][0], OE[l][1]);
          puts(" add:");
          if (f == 1) {
            OE[noe][0] = Eli[i][0];
            OE[noe][1] = OE[noe + 1][1] = k;
            assert(0 < XYZcone(Y[Eli[i][0]], Y[k], Y[Eli[i][1]]));
            OE[noe + 1][0] = Eli[i][1];
          } // OE[noe/noe+1]=1st split(E_i)
          else {
            for (l = noe; l < noe + f; l++) // split OE[noe<= ... <noe+f]
              if (XYZcone(Y[OE[l][0]], Y[k], Y[OE[l][1]]) > 0)
                break; // split OE_l by Y_k
            ...
            OE[noe + f][1] = OE[l][0] = k;
            for (l++; l < noe + f; l++)
              assert(-1 == XYZcone(Y[OE[l][0]], Y[k], Y[OE[l][1]]));
          }
          if (f == ien[i])
            noe += f + 1;
        }
...
//	Eli[i][1]);for(l=0;l<ien[i];l++)printf("%d ",IEli[i][l]); puts("");}
    ...
    for (i = 0; i < noe; i++) {
      int face[VERT_Nmax]; //// make CHAMBERS CR[]
      face[0] = OE[i][0];
      face[f = 1] = OE[i][1];
      do {
        int fit = 0; // assemble face
        for (j = i + 1; j < noe; j++)
          if (OE[j][0] == face[f]) { // search fitting edges
            if (XYZproduct(Y[face[f - 1]], Y[face[f]], Y[OE[j][1]]) <= 0)
              continue;
            if (fit)
              if (XYZproduct(Y[face[f]], Y[OE[k][1]], Y[OE[j][1]]) <= 0)
                continue;
            fit = 1;
            k = j;
          }
      ...
      if (f == 3)
        for (j = 0; j < 3; j++)
          CR[ncr][j] += Y[face[1]][j]; // chamber ray CR
#if (TRACE_TRIANGULATION)
      printf("face[i=%d]=", i);
      for (j = 0; j <= f; j++)
        printf("%d ", face[j]);
  ...
  for (i = 0; i < ncr; i++) {
    Inci64 *S = CT[*nmt], U = 0; // TRIANGulate forall chambers
    Long *Q = CR[i];
    nt[*nmt] = 0;
    for (j = 0; j < tn; j++) {
      Long a, b, c;
      ...
      if (ChamberTriangle) {
        for (a = 0; a < nrp[Tli[j][0]]; a++)
          for (b = 0; b < nrp[Tli[j][1]]; b++)
            for (c = 0; c < nrp[Tli[j][2]]; c++) {
              S[nt[*nmt]] = C - makeN(F[Z[R[Tli[j][0]][a]]]) -
                            makeN(F[Z[R[Tli[j][1]][b]]]) -
                            makeN(F[Z[R[Tli[j][2]][c]]]);
              U |= S[nt[*nmt]++];
            }
      }
    }
...
int Compatible_Tri(Inci64 CA, Inci64 CB, int a, Inci64 *A, int b, Inci64 *B,
                   int p) {
  Inci64 I = CA & CB, IA[VERT_Nmax], IB[VERT_Nmax];
  int i, ia = 1, ib = 1;
  *IA = (*A) & I;
  for (i = 1; i < a; i++) {
    int j;
...
void GKZsubdivide(Inci64 *F, int f, PolyPointList *P, int p, int *Tp, int *ntp,
  ...
  do {
    ...
    if (comptri) {
      for (k = 1; k < nmf; k++)
        if (nmt[k] > 1) // check compatib
          for (j = 0; j < k; j++)
            if (nmt[j] > 1)
              if (Inci64_abs(C[j] & C[k]) > 3) // triangulations
                if (!Compatible_Tri(C[j], C[k], _nt[j], _CT[j], _nt[k], _CT[k],
                                    p))
                  goodtri = 0;
    }
    if (goodtri) {
      for (j = 0; j < nPS; j++)
        for (k = 0; k < nmf; k++)
          if (Inci64_LE(T[j], C[k])) {
            mt += Induce_Facet_Tri(_CT[k], _nt[k], F[ntp[j]], T[j], &MT[mt], p,
                                   d);
            break;
          }
...
void Subdivide(PolyPointList *P, int v, Inci64 I[], int p, Inci64 *T, int *t,
   ...
   * there are nPS=3 non simplicial facets, precisely the 2nd, 4th and 5th
   * facet.
   * */
  for (i = 0; i < ni; i++)
    T[i] = I[i];
  ...
  if (num_facetIPs > 0 && _Flag->H) {
    if (num_facetIPs == 1)
      printf("WARNING: there is %d facet-IP ignored in the triangulation. This "
   ...
   */
  for (i = v; i < p; i++) {
    for (j = 0; j < ni; j++) // find non-vertex positions:
      if (getN(i, I[j]))
        C[i - v][(c[i - v])++] = j; // C[i][]=facets containing i
    if (i - v > 2) {
...
int SectionCount(PolyPointList *PN, int k, PolyPointList *PM) {
  int i, n = 0;
  for (i = 0; i < PM->np; i++) {
    int j;
    for (j = 0; j < PN->np; j++) {
...
#endif
void NewtonMonomial(Long *X, int d) {
  int num = 0, den = 0, i, t = NewtonMonomCOORD == 1;
  for (i = 0; i < d; i++)
    if (X[i] > 0)
      num++;
    else if (X[i])
...
void HyperSurfDivisorsQ(PolyPointList *_P, VertexNumList *V, EqList *E,
   ...
   * Should read: Permutes the Facets-IP to the rights of the PPL.
   *
   */
  ...
  if (!_Flag->M)
    for (i = 0; i < cp; i++) { /* facet IPs to the end */
      int fi = 0;
  ...
  if (_Flag->P) {
    ...
    for (i = 0; i < F->nw;
         i++) { /* i runs over # weight relations = IP-simplices */
      int cd = _P->n - cp + 1;
  ...
  for (i = 0; i < cp; i++) {
    ...
    if (fi == 2) {
      ...
      for (j = 1; j < d; j++)
        g = NNgcd(g, E->e[FI[1]].a[j] - E->e[FI[0]].a[j]);
      Dh0[i] = g;
    }
  }
  /*******************************************************************/
  ...
  if (_Flag->K) {
     ...
     */
    if (_Flag->M)
      puts("Option -K should not be combined with option -M");
    else {
      printf("KreuzerPoly=");
      for (i = 0; i < V->nv; i++) {
        if (i)
...
#if (PRINT_MONOMIALS)
  for (j = 0; j < E->ne; j++) {
    for (i = 0; i < cp; i++) /* print monomials */
      printf(" %4ld", Eval_Eq_on_V(&(E->e[j]), _P->x[i], _P->n));
    puts(" monomial");
  }
#endif
  ...
  // formula enumerates $m$ with $<m,v_j>+a_j\ge0$ for sections of $\sum
  // a_jD_j$.
 ...
 *                   MORI CONE: mori.x -DM
 ****************************************************************
 * The following functions are needed for option -M
 *
 * Example:
 * echo -e "2 8\n0 -1 -1  0 1 -1 -1 0\n0  2 -1 -1  0  0 1 1\n1\n7 10011000
 * 10100100 10110000 10000110 11000010 10001001 11000001" | ./mori.x -DM
 *
 * These routines are those contained in the old mori.c
 * of version palp-1.1. It would be desirable to clean up this
 * part and harmonically integrate it in the code above:
 * 1) some functions do very similar things as others already
 * contained in the code above (e.g. Print_Mori_Old <-> Print_Mori)
 * 2) note the use of INCI instead of Inci64
 *
 ****************************************************************/
...
void INCI_2_VNL(Inci64 X, VertexNumList *_V, int n) {
  _V->nv = 0;
  while (!Inci64_EQ_0(X)) {
    n--;
    if (Inci64_M2(X))
...
/*needed from Read_Tri*/
void Test_INCI(int *nI, Inci64 *ILi, int p) {
  Inci64 X = ILi[0];
  VertexNumList V;
  int i;
  for (i = 1; i < *nI; i++)
    X = Inci64_AND(X, ILi[i]);
  INCI_2_VNL(X, &V, p);
  if ((V.nv != 1) || (V.v[0] != 0)) {
    IDerr();
...
void Print_Mori_Old(PolyPointList *P, int nI, Inci64 *I) {
    ...
    for (j = 0; j < i; j++)
      if (Inci64_abs(Inci64_AND(I[i], I[j])) == d) {
        ...
        for (a = 0; a < p; a++)
          R.x[r][a] = 0;
        if (0 == Z[0]) {
          printf("Error: Z[0]==0 for I[%d]&I[%d] !\n", j, i);
          for (a = 0; a <= d; a++) {
  ...
  for (i = 0; i < nv; i++) {
    for (j = 0; j < nv; j++)
      if (Inci64_LE(IE[i], IE[j]) && !Inci64_EQ(IE[i], IE[j]))
        break;
    if (j == nv)
      m[nm++] = i;
  }
...
long long Compute_Abi(PolyPointList *P) {
  int i = 1, j = P->np / 2;
  long long bico = P->np, Abi;
  if (j > P->n)
    j = P->n + 1;
  while (i < j) {
    bico *= (P->np - i);
...
void ComputeStanleyReisner(PolyPointList *P, int nI, Inci64 *I, int *NrInz,
  ...
  for (i = 1; i < v; i++)
    for (j = 0; j < i; j++)
      M[m++] = Inci64_OR(IV[i], IV[j]);
  for (i = 0; i < m; i++) {
    for (j = 0; j < nI; j++)
      if (Inci64_LE(M[i], I[j]))
        break;
    if (j == nI) {
      SRG[g++] = M[i];
      nG[2]++;
  ...
  for (d = 3; d <= P->n + 1; d++) {
    ...
    for (i = 0; i < m; i++) {
      int l = 0;
      while (!Inci64_LE(IV[l], M[i]))
        N[n++] = Inci64_OR(IV[l++], M[i]);
    }
    for (i = 0; i < n; i++) {
      for (j = 0; j < nI; j++)
        if (Inci64_LE(N[i], I[j]))
          break;
      if (j == nI) {
        for (j = 0; j < g; j++)
          if (Inci64_LE(SRG[j], N[i]))
            break;
        if (j == g) {
          SRG[g++] = N[i];
          nG[d]++;
...
void TriList_to_MoriList(PolyPointList *_P, FibW *F, MORI_Flags *_Flag) {
  ...
  for (n = 0; n < Ntri; n++) {
    ...
    /*** From here starts the code needed for composite options like
         -DM -i, -t, -b, -c, -d, -a, -H ***/
    if (_Flag->i || _Flag->t || _Flag->b || _Flag->c || _Flag->d || _Flag->a ||
        _Flag->H) {
      SR.nmax = T.nmax = VERT_Nmax;
      SR.d = T.d = _P->n;
      SR.v = T.v = _P->np;
      ...
      */
      ...
      T.v = SR.v = T.v - 1;
      // reduce by one the number of entries in the incidences

      // Diagnostics: prints the triangulation without the origin entry
      //
      //	int j;
      //	puts(" ");
      //	for(i=0;i<T.n;i++){
      ...
      //(T.I[i]>>(T.v-j-1))%2);} 		fprintf(outFILE, " T[%d]=%d", i, T.I[i]);//
      //diagnostics
      //	}
      ...
      for (i = 0; i < T.n; i++)
        T.I[i] = Inci64_revert(T.I[i], T.v);
      for (i = 0; i < SR.n; i++)
        SR.I[i] = Inci64_revert(SR.I[i], SR.v);

      // Diagnostics: prints the SR-ideal without the origin entry
      //
      //	puts(" ");
      //	for(i=0;i<SR.n;i++){
      //		if(i)fprintf(outFILE," ");
      //			for(j=0;j<SR.v;j++){fprintf(outFILE, "%d",
      //(SR.I[i]>>(SR.v-j-1))%2);} 		fprintf(outFILE, " SR[%d]=%d", i, SR.I[i]);//
      //diagnostics
      //	}
      ...
      // Diagnostics: print the CWS computed via IP_Simplex_Fiber
      //
      //			for(i=0; i< F->nw;i++){
      //				for(j=0;j<cp;j++){
      //					fprintf(outFILE, " %d",
