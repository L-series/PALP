...
int SL2Z_Aux_Make_Poly_NF(Long X[][VERT_Nmax], int *n, int *nv) {
  SL_Long S[POLY_Dmax][POLY_Dmax];
  return SL2Z_Make_Poly_NF(X, n, nv, S);
}
void SL2Z_Make_Poly_UTriang(PolyPointList *P) {
  if (VERT_Nmax < P->np)
    puts("Triang Form requires VERT_Nmax>=#Points");
  else {
    int i, j;
    Long UTF[POLY_Dmax][VERT_Nmax];
    for (i = 0; i < P->np; i++)
      for (j = 0; j < P->n; j++)
        UTF[j][i] = P->x[i][j];
    SL2Z_Aux_Make_Poly_NF(UTF, &P->n, &P->np);
    for (i = 0; i < P->np; i++)
      for (j = 0; j < P->n; j++)
        P->x[i][j] = UTF[j][i];
  }
...
GL_Long GL_W_to_GLZ(GL_Long *W, int d, GL_Long **GLZ) {
  ...
  for (i = 2; i < d; i++) {
    ...
    {
      GL_Long *Y = GLZ[j], rB = GL_RoundQ(B[j], Y[j]),
              rE = GL_RoundQ(E[j], Y[j]);
      int n;
      for (n = 0; n <= j; n++) {
        B[n] -= rB * Y[n];
        E[n] -= rE * Y[n];
...
int GLZ_Make_Trian_NF(Long X[][VERT_Nmax], int *n, int *nv,
                      GL_Long G[POLY_Dmax][POLY_Dmax]) {
  int i, j, C = -1, L;
  GL_Long g, W[POLY_Dmax], NF[POLY_Dmax][VERT_Nmax], *_G[POLY_Dmax],
      NG[POLY_Dmax][POLY_Dmax];
  for (i = 0; i < *n; i++)
    _G[i] = NG[i];
  for (i = 0; i < *n; i++)
    for (j = 0; j < *n; j++)
  ...
  for (L = 0; L < *n; L++) {
    ...
    if (L != p[0])
      for (i = 0; i < *n; i++) /* swap lines G[L] <-> G[p[0]] */
      {
        GL_Long A = G[L][i];
        G[L][i] = G[p[0]][i];
        G[p[0]][i] = A;
...
void TEST_rVM_VPM(int *d, int *v, int *f, Long X[POLY_Dmax][VERT_Nmax],
  ...
  for (i = 0; i < *v; i++) {
    for (j = 0; j < *d; j++)
      if (abs(X[j][i]) > X_Limit)
        err = X[j][i];
    for (j = 0; j < *f; j++)
      if (abs(x[j][i]) > VPM_Limit)
        err = x[j][i];
  }
  if (err) {
...
void Aux_vNF_Init(vNF *_X, Long x[][VERT_Nmax], PERM *CL, int *S, int *_ns) {
  ...
  q = CL;
  *q = P;
  b = *x; /* P=CL[ns-1] StartPerm; maximize 0-th line */
  for (j = 1; j < _X->nv; j++)
    if (b[q->C[0]] < b[q->C[j]])
      swap(&q->C[0], &q->C[j]);
  for (i = 1; i < _X->nv; i++) {
  ...
  for (nn = 1; nn < _X->nf; nn++) /* maximize nn-th line */
  {
    ...
    p = &CL[*_ns];
    *p = P;
    y = x[nn]; /* nb=*q=*b=best, nn=*p=*y=next */
    {
      int m = 0;
      for (j = 1; j < _X->nv; j++)
        if (y[p->C[m]] < y[p->C[j]])
    ...
    if ((d = y[p->C[0]] - b[q->C[0]]) < 0)
      continue; /* d<0 => forget this */
    for (i = 1; i < _X->nv; i++) {
      int m = i;
      for (j = i + 1; j < _X->nv; j++)
        if (y[p->C[m]] < y[p->C[j]])
  ...
  y = x[CL->L[0]];
  S[0] = 0;
  for (i = 1; i < _X->nv; i++) /* compute S */
    if (y[CL->C[i]] == y[CL->C[i - 1]])
      ++S[S[i] = S[i - 1]];
    else
      S[i] = i;
}

int Aux_XltY_Poly_NF(Long X[][VERT_Nmax], Long Y[][VERT_Nmax], int *n,
                     int *nv) {
  int i, j;
  Long d; /* return "X < Y" */
  for (i = 0; i < *n; i++)
                 ...
                 PERM *CL, int *ns, Long VPM_NF[VERT_Nmax][VERT_Nmax]) /* out */
{
  ...
  *ns = 1;
  ...
  _X->ns = *ns;
  for (i = 0; i < _X->nv; i++) /* write VPM-NF to _X */
  {
    for (j = 0; j < _X->nf; j++) /* _X->x */
      VPM_NF[j][i] = x[CL->L[j]][CL->C[i]];
  }
  if (nsF)
                 ...
                 Long pNF[POLY_Dmax][VERT_Nmax]) /* 1 if reflexive */
{
  ...
  Eval_Poly_NF(&d, &v, &f, VM, VPM, pNF, 0);
  return ref;
}
...
int Make_Poly_Sym_NF(PolyPointList *_P, VertexNumList *_V, EqList *_F,
  ...
  if (traced)
    Eval_Poly_NF(&_P->n, &_V->nv, &_F->ne, VM, VPM, NF, 1);
  Make_VPM_NF(v, f, VPM, CL, &ns, VPM_NF);
  New_pNF_Order(v, f, CL, &ns, VPM_NF);
  ...
  *SymNum = -t;
  ...
  C = CL[i].C;
  for (t = 0; i < ns; i++)
    if (CL[i].s) /* inv Perm: C[c0[i]]=C[i] */
    {
      for (j = 0; j < *v; j++)
        V_perm[t][C[j]] = CL[i].C[j];
      t++;
    }
  if (*SymNum < SYM_Nmax) {
    int *s = V_perm[*SymNum];
...
int Improve_Coords(PolyPointList *_P, VertexNumList *_V) {
  ...
  for (i = 0; i < _P->n; i++)
    for (j = 0; j < _V->nv; j++)
      V[i][j] = _P->x[_V->v[j]][i];
  if (!SL2Z_Make_Poly_NF(V, &(_P->n), &(_V->nv), S))
    return 0;
  for (j = 0; j < _P->np; j++) {
    for (i = 0; i < _P->n; i++) {
      int k = _P->n;
...
void INV_GLZmatrix(GL_Long G[][POLY_Dmax], int *d, GL_Long Ginv[][POLY_Dmax]) {
  ...
  return;
NoGLZ:
  fputs("No GLZ-Matrix in INV_GLZmatrix:", stderr);
  for (x = 0; x < *d; x++) {
    for (y = 0; y < *d; y++)
...
Long V_to_G_GI(Long *V, int d, Long G[][POLY_Dmax], Long GI[][POLY_Dmax]) {
  ...
  g = GL_V_to_GLZ(AV, P, d);
  INV_GLZmatrix(AG, &d, AGI);
  for (i = 0; i < d; i++)
    for (j = 0; j < d; j++) {
      G[i][j] = AG[i][j];
      GI[i][j] = AGI[i][j];
...
int TriMat_to_Weight(GL_Long T[][POLY_Dmax], int *p, int r, int *s, int *nw,
                     Long W[][VERT_Nmax], int *Wmax) {
  int i, j;
  Long g, a, b, x[POLY_Dmax + 1], *X = W[*nw];
  if (0 <= (b = T[r][r - 1]))
    return 0;
  g = Fgcd(a = T[r - 1][r - 1], -b);
  x[r - 1] = -b / g;
  x[r] = a / g;
  for (j = r - 2; 0 <= j; j--) {
    a = T[j][j];
...
void Remove_Identical_Points(PolyPointList *P) {
  ...
  for (p = 0; p < P->np; p++) {
    for (s = 0; s < r; s++)
      if (0 == XmY_vecdiff(P->x[p], P->x[s], P->n))
        break;
    if (s == r) {
      if (r < p)
        for (i = 0; i < P->n; i++)
...
{
  int i, j, k, r = 0;
  GL_Long B[POLY_Dmax][POLY_Dmax], *b[POLY_Dmax], *g[POLY_Dmax];
  for (i = 0; i < *d; i++) {
    b[i] = B[i];
    g[i] = G[i];
  }
 ...
 */
Long Simp_Vol_Barycent(PolyPointList *A, Long VM[][VERT_Nmax], Long *B,
                       Long *N) {
  int i, j;
  Long I = 0;
  *N = A->np;
...
Long SimplexVolume(Long *V[POLY_Dmax + 1], int d) {
  Long I = 1, VM[POLY_Dmax][VERT_Nmax];
  int i, p;
  for (i = 0; i < d; i++)
    for (p = 0; p < d; p++)
      VM[i][p] = V[p][i];
...
Long Aux_Vol_Barycent(PolyPointList *A, VertexNumList *V, EqList *E, Long *_B,
  ...
  Equation Eq[VERT_Nmax];
  if (A->np == 1 + D)
    return Simp_Vol_Barycent(A, F, _B, _N);
  p = A->np - 1;
  *_N = 1;
  for (i = 0; i < D; i++) {
    _B[i] = A->x[0][i]; /* choose origin _B[] */
    for (j = 0; j < A->np; j++)
      A->x[j][i] -= _B[i];
  }
  ...
  for (e = 0; e < ne; e++) {
    ...
    GL_Long G[POLY_Dmax][POLY_Dmax], GI[POLY_Dmax][POLY_Dmax];
    int f = 0;
    for (i = 0; i < p; i++)
      if (Eval_Eq_on_V(&Eq[e], P[i], D) == 0) {
        for (j = 0; j < D; j++)
          F[j][f] = P[i][j];
        f++;
      }
    for (j = 0; j < D; j++)
      Ze[j] = F[j][0]; /* choose origin Ze[] */
    for (i = 0; i < f; i++)
      for (j = 0; j < D; j++)
        F[j][i] -= Ze[j];
...
int ZeroSum(Long *A, Long *B, int d) {
  while (d--)
    if (A[d] + B[d])
      return 0;
  return 1;
...
int SemiSimpleRoots(PolyPointList *P, EqList *E, Long **R) {
  ...
  for (p = 0; p < P->np; p++) {
    int z = 0;
    for (e = 0; e < E->ne; e++)
      if (0 == Eval_Eq_on_V(&E->e[e], P->x[p], P->n))
        z++;
    if (z == 1)
      R[N++] = P->x[p];
  }
...
void Aux_MinNonNeg_UT(Matrix M, Matrix G, int c, int r, int d, Long D) {
  int i, j;
  for (i = 0; i < r; i++) {
    Long X = VxV(G.x[i], M.x[c], d), R = X / D;
    if (X - R * D < 0)
      R -= 1;
    for (j = 0; j < d; j++)
      G.x[i][j] -= R * G.x[r][j];
...
int SimpUnimod(PolyPointList *P, VertexNumList *V, EqList *E, int vol) {
  ...
  for (v = 0; v < V->nv; v++) {
    int e = 0, i = 0;
    Long *X = P->x[V->v[v]], *Y[POLY_Dmax + 1];
    for (e = 0; e < E->ne; e++)
...
int SimpUnimod_M(PolyPointList *P, VertexNumList *V, EqList *E, int vol) {
  ...
  for (e = 0; e < E->ne; e++) {
    ...
    Long *Y[POLY_Dmax + 1];
    for (v = 0; v < V->nv; v++)
      if (0 == Eval_Eq_on_V(&E->e[e], P->x[V->v[v]], d)) {
        if (i == d)
          return 0;
        Y[i++] = P->x[V->v[v]];
      } /* simplicial */
...
{
  int i, j, k, r = 0, det = 1;
  GL_Long B[POLY_Dmax][POLY_Dmax], *b[POLY_Dmax], Y[POLY_Dmax][POLY_Dmax],
...
{
  int i, j, k, r = 0, det = 1;
  GL_Long B[POLY_Dmax][POLY_Dmax], *b[POLY_Dmax], G[POLY_Dmax][POLY_Dmax],
      *g[POLY_Dmax];
  for (i = 0; i < d; i++) {
    b[i] = B[i];
    g[i] = G[i];
...
int PyramidIP(PolyPointList *P, VertexNumList *V, EqList *E, FaceInfo *FI) {
  ...
  for (i = 0; i < P->np; i++) {
    ...
    int e, eq = 0;
    for (e = 0; e < E->ne; e++) {
      if (0 == Eval_Eq_on_V(&E->e[e], Xi, P->n))
        eq++;
    }
    if (eq == 1)
      if ((X[0] - Xi[0]) % 2 == 0)
...
#endif
    if (1 < RelativeSimplexVolume(X, 3, P->n)) {
      nonbasic++;
      free(_FI);
      return 0;
    }
  ...
  while (Read_CWS_PP(CW, P)) /* nis=noinvss s=sum ks=ksum bcz=bary0 ssr(oot) */
  {
    ...
    *c = 0;
    ...
    if (NR)
      for (i = 0; i < E->ne; i++)
        d[i] = E->e[i].c;
    LatVol_Barycent(P, V, C, &N);
    bcz = (N == 0);
    if (bcz)
      bary++; /*BaryCenterZero*/

    Make_VEPM(P, V, E, PM);
    Complete_Poly(PM, E, V->nv, P);
    ssr = SemiSimpleRoots(P, E, root);
    if (ssr)
      ssroot++;
    if (ssr < 0)
      nofip++;
    ...
    for (i = 0; i < A->n; i++) {
      for (j = 0; j < A->np; j++)
        S += A->x[j][i];
      if (S) { /* if(bcz) Print_PPL(P,"bary=0 for Psum!=0"); */
        break;
      }
    }
    if (S) {
    ...
    if (S != 0)
      if (r >= KP_PRINT) {
        ...
        for (i = 0; i < P->n; i++) {
          for (j = 0; j < V->nv; j++)
            fprintf(stderr, "%3ld%s", P->x[V->v[j]][i],
                    (j + 1 < (V->nv)) ? " " : "\n");
        }
        fflush(0);
      }
    ...
    if (((SSR_PRINT) && (ssr > 0)) || ((SSR_PRINT > 1) && ssr) ||
        ((BARY_PRINT) && bcz) || ((ZEROSUM_PRINT == 1) && s) ||
        ((ZEROSUM_PRINT == 2) && ks)) {
      int nr = (ssr + 1) ? ssr : 0;
      printf("%d %d    v=%d p=%d roots=%d  ", P->n, V->nv + nr, V->nv, P->np,
             nr);
      puts(c);
      for (i = 0; i < P->n; i++) {
        for (j = 0; j < V->nv; j++)
          printf("%2ld ", P->x[V->v[j]][i]);
        printf("  ");
        for (j = 0; j < nr; j++)
          printf(" %2ld", root[j][i]);
        puts("");
...
void IPS_Rec_New_Vertex(Long PM[][POLY_Dmax], int *p, int *d, int *nw,
                        ...
                        GL_Long *GI[POLY_Dmax][POLY_Dmax], GL_Long **GN,
                        GL_Long T[][POLY_Dmax], int *s, int r, FibW *FW,
                        int *CD) {
  int i, j, k, *n = &s[r];
  GL_Long *X = T[r]; /* printf("called r=%d\n",r); */
  for (*n = s[r - 1] + 1; *n < *p; (*n)++) {
    Long *P = PM[*n];
    for (i = 0; i < *d; i++) {
      X[i] = 0;
...
int Fiber_Ref_Check(Long PM[][POLY_Dmax], int *d, /*int *p,*/ int *v,
  ...
  return Ref_Check(A, &V, &E);
}
void Add_Ref_Fibers(Long PM[][POLY_Dmax], int *d, int *v, int *s,
                    GL_Long G[][POLY_Dmax][POLY_Dmax], int *n, PolyPointList *A,
                    int r) {
  int i, j, c, l;
  Long X[VERT_Nmax][VERT_Nmax], x; /* n==nf */
  GL_Long Ginv[POLY_Dmax][POLY_Dmax];
  for (i = 0; i < r; i++)
    for (j = 0; j < *d; j++)
      X[j][i] = PM[s[i]][j];
  PM_to_GLZ_for_UTriang(X, d, &r, G[*n]);
  INV_GLZmatrix(G[*n], d, Ginv);
...
void Fiber_Rec_New_Point(PolyPointList *_P, int *v, /* already selected r pts */
  ...
  for (*n = s[r - 1] + 1; *n < _P->np - (*fdim) + r; (*n)++) {
    ...
    if (j < *d) /* non-zero :: rank increased */
    {
      ...
      if (r < (*fdim) - 1)
        Fiber_Rec_New_Point(_P, v, G, GI, GN, T, s, r + 1, F, fdim);
      else
        Add_Ref_Fibers(_P->x, d, v, s, F->G, &F->nf, &F->F, r + 1);
    }
  }
}
void Reflexive_Fibrations(PolyPointList *P, int nv, ek3fli *F, int fdim) {
  ...
  for (n = 0; n < d; n++) {
    for (i = 0; i < d; i++) /* G=G-list GN=G-new GI->lines of GX */
      if (i < n)
        GI[n][i] = GI[n - 1][i];
      else
        GI[n][i] = GX[j++];
...
void Elliptic_K3_Fibration(PolyPointList *P, int nv, int edim) {
  ...
  for (e = 0; e < F->nf; e++)
    for (c = 0; c < P->np - 1; c++) {
      int i, j, nz = 0, newf = 1;
      GL_Long *b[POLY_Dmax], X[POLY_Dmax];
      for (i = edim; i < P->n; i++)
        if ((X[i] = GxP(F->G[e][i], P->x[c], &P->n)))
          nz = 1;
      if (nz) /* find new B; compare/add B; BxG-FiberRefCheck; print */
      {
        ...
        for (i = 0; i < nb; i++) /* check if already found */
        {
          int l, C = 0;
          nz = 0;
          for (j = 1; j < cd; j++) /* for(C=0;C<r;C++) */
          {
            GL_Long x = 0;
            for (l = 0; l < *d; l++)
              x += F->B[i][j][l] * Binv[l][C];
            if (x) {
              nz = 1;
              break;
            } /* r->1, d->cd */
...
void Print_Fiber_PolyData(PolyPointList *P, VertexNumList *V, Long *W, int w,
  ...
  if (CD || ((cd > 0) && (cd < 3) && ((P->n) - cd > 1))) {
    ...
    F->np = P->np;
    F->n = D;
    for (p = 0; p < w; p++) /* now check ref in N only */
      if (W[p]) {
        for (i = 0; i < D; i++)
          F->x[s][i] = P->x[p][i];
        s++;
...
void Check_New_Fiber(Long PM[][POLY_Dmax], int *d, int *s, int r, FibW *F) {
  ...
  for (i = 0; i < r; i++)
    for (j = 0; j < *d; j++)
      X[j][i] = PM[s[i]][j];
  PM_to_GLZ_for_UTriang(X, d, &r, F->G[*n]);
  INV_GLZmatrix(F->G[*n], d, Ginv);
  for (i = 0; i < *n; i++)
    if (r == F->r[i]) /* check if already found */
    {
      int newfib = 0;
      for (j = r; j < *d; j++)
        for (c = 0; c < r; c++) {
  ...
  for (i = 0; i < F->nv; i++)
    for (j = 0; j < *d; j++)
      F->P->x[i][j] =
          GxP(F->G[*n][j], PM[i], d); /* reflexivity of projection */
  {
    VertexNumList V;
    EqList E;
...
void Print_Fibrations(PolyPointList *P, FibW *F) {
  ...
  EqList E;
  for (n = 0; n < F->nf; n++) {
    int s[VERT_Nmax], i, r = F->r[n], *d = &P->n, l, c = 0, N;
    for (i = 0; i < P->np - 1; i++) {
      for (l = r; l < *d; l++)
        if (GxP(F->G[n][l], P->x[i], d))
          break;
      if (l == *d) {
        for (l = 0; l < r; l++)
          F->P->x[c][l] = GxP(F->G[n][l], P->x[i], d);
        s[c++] = i;
      }
    }
    F->P->np = c;
...
void IP_Fiber_Data(PolyPointList *PD, PolyPointList *AuxP,
  ...
  F->PS = F->ZS = 0;
  F->nv = nv;
  IP_Simplex_Fiber(PD->x, PD->np - 1, PD->n, F, FIB_Nmax, CD);
  *nf = F->nf;
  for (i = 0; i < *nf; i++) {
    fd[i] = F->r[i];
    for (j = 0; j < PD->n; j++)
...
void Normalize_Diagonal(int *d, Long *D, GL_Long **G) {
  ...
  for (a = 0; a < *d - 1; a++)
    for (b = a + 1; b < *d; b++)
      if (D[b] % D[a]) {
        GL_Long A, B, g = GL_Egcd(D[a], D[b], &A, &B), X = -D[b] / g,
                      Y = D[a] / g, L;
        D[b] *= D[a] / g;
        D[a] = g;
        for (i = 0; i < *d; i++) {
          L = A * G[a][i] + B * G[b][i];
          G[b][i] = X * G[a][i] + Y * G[b][i];
          G[a][i] = L;
        }
      }
  for (i = 1; i < *d; i++)
...
int GL_Lattice_Basis(int d, int p, Long *P[POLY_Dmax], /* return index */
  ...
  for (L = 0; L < d - 1; L++) {
    ...
    g = 0;
    for (C = 0; C < p; C++)
      if ((a = labs(AuxColGCD(&d, L, GM, P[C])))) {
        if (a == 1) {
          for (l = L; l < d; l++)
            V[l] = GxP(G[l], P[C], &d);
          g = GL_V_to_GLZ(&V[L], B, d - L);
          G_2_BxG(G, B, &d, &L);
          c = p;
          break; /* P[c] -> B, G.B */
    ...
    if (c < p) /* check for divisibility */
    {
      ...
      for (C = 0; C < p; C++)
        if ((a = GxP(G[L], P[C], &d)) % g) /* improve line */
        {
          Long vg, va;
          V[L] = Egcd(g, a, &vg, &va);
          for (l = L + 1; l < d; l++)
  ...
  D[L] = g;
  index *= g;
  Normalize_Diagonal(&d, D, G);
  INV_GLZmatrix(GM, &d, BM);
  return index;
}
...
{
  int i, j, I;
  GL_Long GLG[POLY_Dmax][POLY_Dmax], Ginv[POLY_Dmax][POLY_Dmax];
                         ...
                         Long G[][POLY_Dmax]) /* normalize and diagonalize Z */
{
  ...
  for (i = 0; i < *m - 1; i++)
    for (j = *m - 1; j > i; j--) /* reduce: (g1.g2)  m=lcm(m1,m2) */
    {
      int mi, mj;
      Long *Zi, *Zj;
      g = Fgcd(M[i], M[j]); /* g2^(m2/m') m'=gcd(m1,m2) */
      mi = M[i] / g;
      mj = M[j] / g;
      Zi = Z[i];
      Zj = Z[j];
  ...
  r = PM_to_GLZ_for_UTriang(A, d, m, GT);
  INV_GLZmatrix(GT, d, Ginv);
  for (i = 0; i < *d; i++)
    for (j = 0; j < *d; j++)
...
void Print_QuotientZ(int *r, int *p, Long Z[POLY_Dmax][VERT_Nmax], Long *M) {
  int i, j;
  fprintf(stderr, "Z-action:\n");
  for (i = 0; i < *r; i++) {
    for (j = 0; j < *p; j++)
      fprintf(stderr, "%5ld ", Z[i][j]);
    fprintf(stderr, "  /Z%ld\n", M[i]);
  }
}
void Normalize_QuotientZ(int *r, int *p, Long Z[POLY_Dmax][VERT_Nmax],
                         Long *M) {
  int i, j = 0, k;
  ...
  *r = j; /* drop trivial factors */
  ...
  for (i = 0; i < *r - 1; i++)
    for (j = *r - 1; j > i; j--)
      if (M[i] % M[j]) /* Mi -> L; Mj -> g */
      {
        Long g = Fgcd(M[i], M[j]), *Zi = Z[i], *Zj = Z[j];
        int mi = M[i] / g, mj = M[j] / g;
        M[i] *= mj;
        for (k = 0; k < *p; k++)
          Zi[k] = (mj * Zi[k] + mi * Zj[k]) % M[i];
        if (g > 1) /* compute G2 */
        {
          Long A, B;
...
void Test_Effective_Zaction(int *r, int *d, Long Z[POLY_Dmax][VERT_Nmax],
                            Long *M) {
  int i, j;
  Long g;
  for (i = 0; i < *r; i++) {
...
Long AuxLinRelGPZ(Long *A, int *j, Long *D, int *d, Long GP[][POLY_Dmax],
                  int *p, Long Z[][VERT_Nmax]) {
  Long s = 0, g = 0;
  int i, l;
  for (l = 0; l < *p; l++) {
    A[l] = (l == (*j));
    for (i = 0; i < *d; i++)
      A[l] -= (GP[*j][i] / D[i]) * Z[i][l];
    g = (g) ? NNgcd(A[l], g) : A[l];
  }
  if (g) {
    for (l = 0; l < *p; l++)
...
void Test_Phase(int d, int p, Long *P[], Long Z[][VERT_Nmax], Long *M, int r,
                char *c) {
  int i, j;
  for (i = 0; i < r; i++)
    if (Phase(Z[i], p) % M[i])
...
int ImprovePhase(int L, Long *A, Long *D, int *d, Long GP[][POLY_Dmax], int *p,
  ...
  for (j = *p - 1; 0 <= j; j--) {
    Long a, b, r, l;
    s = AuxLinRelGPZ(A, &j, D, d, GP, p, Z) % m;
    if (s == 0)
      continue;
    if (s < 0)
      s += m;
...
int GL_Lattice_Basis_QZ(int d, int p, Long *P[VERT_Nmax], Long *D, /* index */
  ...
  Long g, a, index = 1, *Y;
  GL_Long V[POLY_Dmax], *G[POLY_Dmax], *B[POLY_Dmax];
  int L, C, tz = (p < VERT_Nmax);
  Long GP[VERT_Nmax][POLY_Dmax], A[VERT_Nmax];
  for (L = 0; L < d; L++) {
    G[L] = GM[L];
    B[L] = BM[L];
    for (C = 0; C < d; C++)
  ...
  if (tz) {
    ...
    for (L = 0; L < d; L++)
      if (Phase(Z[L], p) % D[L] != 0) /* test determinant */
        if (!ImprovePhase(L, A, D, &d, GP, &p, Z))
          if (d < 4) {
            int i, j;
            fprintf(stderr,
                    "\nUnable to remove phase of Z%d. Please send a bug report "
                    "with\nthe ",
                    L);
            fprintf(stderr,
                    "following data to  kreuzer@hep.itp.tuwien.ac.at\n\n");
            fprintf(stderr, "%d %d  Points:\n", d, p);
            for (i = 0; i < d; i++) {
              for (j = 0; j < p; j++)
                fprintf(stderr, " %4ld", P[j][i]);
              fprintf(stderr, "\n");
            }
            for (i = 0; i < d; i++) {
              fprintf(stderr, "Z%d:  ", i);
              for (j = 0; j < p; j++)
                fprintf(stderr, " %4ld", Z[i][j]);
              fprintf(stderr, "  -> %ld /%ld\n", Phase(Z[i], p) % D[i], D[i]);
            }
            for (i = 0; i < d; i++) {
              fprintf(stderr, "GP: ");
              for (j = 0; j < p; j++)
                fprintf(stderr, "%5ld", GP[j][i]);
              fprintf(stderr, "\n");
            }
            fprintf(stderr, "\n");
            exit(0);
    ...
    for (l = 0; l < d; l++) {
      if (D[l] > 1)
        for (c = 0; c < p; c++) /* make Z min nonneg */
          if ((Z[l][c] %= D[l]) < 0)
            Z[l][c] += D[l];
    }
  } else {
...
Long WZ_to_GLZ(Long *W, Long *Waux, int *d, Long **G) /* allows components=0 */
{
  ...
  if (i < *d) {
    ...
    for (i = 0; i < *d; i++)
      fprintf(stderr, "%ld ", W[i]);
    fprintf(stderr, "=W  nonzero=%d<%d\n", r, *d);
    for (i = 0; i < *d; i++) {
...
void C_to_BrxC(Long **B, Long **C, Long *Xaux, int *r,
               int *d) /* B::r_last_coor */
{
  int l, c, n = *d - *r;
  for (c = 0; c < *d; c++) {
...
int VP_2_CWS(Long *V[], int n, int v, CWS *CW) {
  ...
  Long Z[POLY_Dmax][VERT_Nmax], G[POLY_Dmax][POLY_Dmax], M[POLY_Dmax],
      ...
      *B[AMBI_Dmax], *C[AMBI_Dmax], X[AMBI_Dmax];
  if (v > AMBI_Dmax)
    return 0;
  for (j = 0; j < v; j++)
    for (i = 0; i < n; i++)
      VM[j][i] = V[j][i];
  IP_Simplex_Decomp(VM, v, n, &nw, W, FIB_Nmax, 0);
  for (r = 0; r < nw; r++) {
    p[r] = d[r] = 0;
    for (i = 0; i < v; i++) {
      if (W[r][i])
...
void Print_if_Divisible(PolyPointList *P, VertexNumList *V) {
  char divi[99];
  Long g = Divisibility_Index(P, V);
  if (g < 2)
    return;
  sprintf(divi, "divisible by factor=%ld", g);
  Print_VL(P, V, divi);
}
...
void Make_Dilat_Poly(PolyPointList *P, VertexNumList *V, EqList *E, int k,
  ...
  Complete_Poly(MaxDist, E, 1, kP);
  for (e = 0; e < E->ne; e++)
    E->e[e].c /= k;
}

void LatVol_IPs_degD(PolyPointList *P, VertexNumList *V, EqList *E, int g) {
  Long vB[POLY_Dmax], vol, Z;
  int e, j;
  vol = LatVol_Barycent(P, V, vB, &Z);
  printf("vol=%ld, baricent=(", vol);
  for (j = 0; j < P->n; j++)
    printf("%s%ld", j ? "," : "", vB[j]);
  printf(")/%ld\n", Z);
  if (g) {
    ...
    for (j = 0; j < gP->np; j++) {
      ...
      for (e = 0; e < E->ne; e++)
        if (E->e[e].c == 0)
          if (0 == Eval_Eq_on_V(&E->e[e], gP->x[j], P->n))
            cd++;
      if ((cd == 0) || (E->ne == P->n + 1)) {
        for (i = 0; i < P->n; i++)
          printf(" %ld", gP->x[j][i]);
...
void IPs_degD(PolyPointList *P, VertexNumList *V, EqList *E, int g) {
  PolyPointList *gP = (PolyPointList *)malloc(sizeof(PolyPointList));
  int e, j = 0;
  for (e = 0; e < E->ne; e++)
    if (E->e[e].c == 0)
...
void Make_ANF(PolyPointList *P, VertexNumList *V, /* affine normal form */
  ...
  for (i = 0; i < e; i++)
    E->e[i].c = E->e[i].a[d]; /* restore original P,E,V */
  P->n = d;
  P->np = p;
  V->nv = v;
  E->ne = e;
...
void EPrint_VL(PolyPointList *_P, VertexNumList *V, double f) {
  ...
  for (i = 0; i < _P->n; i++) {
    for (j = 0; j < V->nv; j++)
      fprintf(stderr, " %3ld", _P->x[V->v[j]][i]);
    fprintf(stderr, "\n");
  }
}
...
int FindOctahedron(PolyPointList *P, VertexNumList *V, EqList *E) {
  Matrix M, G;
  int n, i, x = 0, X[90 * VERT_Nmax];
  for (n = 1; n < P->np; n++) {
    int l;
    for (l = 0; l < n; l++) {
      for (i = 0; i < P->n; i++)
  ...
  Init_Matrix(&M, x, P->n);
  Init_Matrix(&G, P->n, P->n);
  for (n = 0; n < x; n++)
    for (i = 0; i < P->n; i++)
      M.x[n][i] = P->x[X[n]][i];
  n = (M.d == Make_G_for_GxMT_UT(M, G));
  Free_Matrix(&M);
  Free_Matrix(&G);
  return n;
}
...
int CodimTwoFaceNum(PolyPointList *P, VertexNumList *V, EqList *E) {
  ...
  for (i = 1; i < E->ne; i++)
    for (j = 0; j < i; j++) {
      int k;
      INCI x = INCI_AND(EI[i], EI[j]); /* x=candidate */
      for (k = 0; k < n; k++) {
...
int Fano5d(PolyPointList *P, VertexNumList *V, EqList *E) {
  ...
  for (e = 0; e < E->ne; e++) {
    ...
    int i = 0;
    for (n = 0; n < np; n++)
      if (0 == Eval_Eq_on_V(&E->e[e], P->x[n], d)) {
        FI[e] += 1 << n;
        if (i > d)
          return 0;
        p[i++] = n;
    ...
    for (n = 0; n < i; n++)
      Y[n] = P->x[p[n]];
    if (i == d) {
      if (1 != (SimplexVolume(Y, d)))
        return 0;
    } /* simplicial unimodular */
    else {
      ...
      if (sum || (m2 && p2))
        return 0; /* Batyrev :: inconsistent projection */
      CI[nc] = 0;
      for (i = 0; i <= d; i++) {
        D[p[i]] = 0;
        if (C[i])
      ...
      for (i = 0; i < nc; i++)
        if (CI[nc] == CI[i])
          break; /* already known circuit? */
      if (i == nc) {
        for (i = 0; i < np; i++)
          CC[nc][i] = 0; /* init new circuit */
        if (p2)
          for (i = 0; i <= d; i++)
            CC[nc][p[i]] = -C[i]; /* if(p2) => change signs */
        else
          for (i = 0; i <= d; i++)
            CC[nc][p[i]] = C[i];
        if (p2 || m2) {
  ...
  for (e = 0; e < E->ne; e++) {
    int c, x = 0, X[FanoProjNPmax]; /* BEGIN go over CELLS */
    for (n = 0; n < np; n++)
      if (getNI(n, FI[e]))
        X[x++] = n;
    if (x == d) { /*MakeCellBasisFanoP(X);*/ /* simplical facet case */
      INCIbits I = 0;                        /* BEGIN base change */
      int i, j, k, l, goon = 1, DP[FanoProjNPmax + 1];
      DP[np] = 1;
      for (i = 0; i < d; i++) {
        for (j = 0; j < d; j++)
          M.x[j][i] = P->x[X[j]][i];
        ...
        if (getNI(X[n], CI[c])) { /* drop X[n] */
          ...
          for (z = 0; z < x; z++)
            if (z != n)
              Y[y++] = X[z];
          if (abs(CC[c][X[n]]) ==
              1) { /*MakeCellBasisFanoP(Y)*/ /* call MakeCBF */
            INCIbits I = 0;                  /* BEGIN base change */
            int i, j, k, l, goon = 1, DP[FanoProjNPmax + 1];
            DP[np] = 1;
...
int TempVecUpdate1(int tempvec[], int l, int *atotal_zeiger, int limitwert,
  ...
  while (akt >= 0) {
    if (akt == dpindex) {
      if (((*atotal_zeiger + 1) < limitwert) &&
          ((tempvec[akt] + 1) < singlelimit)) {
...
int CCtest(PolyPointList *P, int d, int np, int nc,
  ...
  for (i = 0; i < nc; i++) /*Ueber alle Circuits laufen*/
  {
    ...
    } else /*Also vom Typ 1 -1 1 0 -1 Schluss 1*/
    {
      ...
      for (j = 0; j < np; j++)
        temp += P->x[j][d] * CC[i][j];
      if (temp != 0) { /*2. Fall: -1 1 -1 0 1 1 stimmt!*/
        temp = 0;
        for (j = 0; j < (np - 1); j++)
          temp -= P->x[j][d] * CC[i][j];
        if ((temp + P->x[np - 1][d]) != 0)
...
int CalculateFano(PolyPointList *P, EqList *E, int d, int np, int na, int aDP,
                  int bDP, int cDP, int s[], int modeDP, int zeronumber, int nc,
                  int CC[FPcirNmax][FanoProjNPmax]) {
  ...
  if (modeDP == 1) {
    ...
    do {
      if (zeronumber <= (d + 1)) {
        ...
        TempVecUpdate1(tempvec, l, &atotal, d + 1, aDP - d, d, &zeronumber));
  } else if (modeDP == 4) {
...
int Make_Fano5d(PolyPointList *P, int *Dpt, EqList *E, /* nc=#Circuits */
  ...
  /*printf("\n1.Fall: Keine DPs\n");*/
  nf = nf + CalculateFano(P, E, d, np, 0, aDP, bDP, cDP, s, 1, 0, nc,
                          CC); /*Keine DPs*/
        ...
        if ((maxVnumber - np) > 1) {
          ...
          for (j = d; j < np - 1; j++) {
            if ((Dpt[j] == 1) && (ZeroSum(P->x[i], P->x[j], d))) {
              ...
              nf = nf + CalculateFano(P, E, d, np, 2, aDP, bDP, cDP, s, 5, 2,
                                      nc, CC);
      ...
      if ((maxVnumber - np) > 1) {
        ...
        for (i = 0; i < d; i++) {
          if (Dpt[i] == 1) {
            ...
            nf = nf +
                 CalculateFano(P, E, d, np, 2, aDP, bDP, cDP, s, 2, 1, nc, CC);
            /*noch 1 DP in 0, ..., d-1 ausser 0-DP*/

            if ((maxVnumber - np) > 2) {

              /*wegen Satz von Casagrande bzw. klar: keine 2 DPs in 0, ...,
               * d-1*/
              ...
              for (j = d; j < np - 1; j++) {
                if ((Dpt[j] == 1) && (ZeroSum(P->x[i], P->x[j], d))) {
                  ...
                  P->x[cDP][d] = 1;
                  /*printf("\n6.Fall: 0 ist DP, 2 DPs, 1 am Anfang
                   * (%d,%d,%d)\n",aDP,bDP,cDP);*/
                  nf = nf + CalculateFano(P, E, d, np, 3, aDP, bDP, cDP, s, 6,
                                          2, nc, CC);
        ...
        for (i = d; i < np - 1; i++) {
          if (Dpt[i] == 1) {
            ...
            /*printf("\n4.Fall: 0 ist DP, 1 DP nicht am Anfang
             * (%d,%d)\n",aDP,bDP);*/
            nf = nf +
                 CalculateFano(P, E, d, np, 2, aDP, bDP, cDP, s, 4, 0, nc, CC);
