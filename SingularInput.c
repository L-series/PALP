...
int Read_HyperSurf(int *he, int divclassnr, int maxline, char filename[20],
                   MORI_Flags *_Flag) {
...
void HyperSurfSingular(PolyPointList *P, triang *T, triang *SR,
  ...
  dprintf(SF, "ring r=0,(t,%s%d", D,
          N + 1); // ring r=0,(t,D1,...,Dp,B1,...,Bp-d),ds;
  for (i = N + 2; i <= p; i++)
    dprintf(SF, ",%s%d", D, i);
  for (i = 1; i <= p - d; i++)
    dprintf(SF, ",%s%d", B, i);
  ...
  if (_Flag->H == 1) {
    ...
    int control =
        Read_HyperSurf(he, divclassnr, 5 * divclassnr, "HEInput.txt", _Flag);
  ...
  dprintf(SF, "number vol=");
  {
    ...
    if ((NN < 0) || (NN > T->n))
      NN = 0;
    //  for(j=1;j<T->n;j++)if(T->I[j]<T->I[NN])NN=j; //avoid D_I.DHySurf=0 for
    //  P3xP3/Z3
    while (i < p) {
      if (getN(i++, T->I[NN])) {
        x[n] = i - 1;
  ...
  if (_Flag->i || _Flag->c) {
    ...
    dprintf(SF, "for(i=1;i<=%d;i++){\n", p - d);
    dprintf(SF, "if(reduce(Jli[i],std(hypideal))==0){nonintersectingJ="
                "nonintersectingJ+list(Jli[i]);}\n");
    dprintf(SF, "}\n");
    dprintf(SF, "if(size(nonintersectingJ)>0){\n");
    dprintf(SF, "  \"SINGULAR -> nonintersecting divisor classes (integral "
                "basis): \";\n");
  ...
  /* Singular procedure to compute the Todd class of a vector bundle given by
   * its Chern character */
  dprintf(SF, "proc Todd(poly ch, int Dim) {\n");
  dprintf(SF, "  poly ToddD = 1;\n");
  dprintf(
      SF,
      "  for(i=2;i<=Dim+1;i++){ToddD=ToddD+(-dli[1]*t)^(i-1)/factorial(i);}\n");
  ...
  dprintf(SF,
          "for(i=1;i<=%d;i++){ChernBottomHypersurface=ChernBottomHypersurface+("
          "-1)^i*HySurf^i;}\n",
          DIM);
  dprintf(SF, "poly ChernHySurf=ChernUp*ChernBottomHypersurface;\n");
  for (j = 0; j < DIM; j++)
    dprintf(SF, "poly c%dHySurf=jet(ChernHySurf,%d)-jet(ChernHySurf,%d);\n",
            j + 1, j + 1, j);
  ...
  if (_Flag->d && DIM > 1) {
    ...
    for (i = 1; i <= p; i++) {
      ...
      dprintf(SF,
              "for(i=1;i<=%d;i++){ChernBottomD%d=ChernBottomD%d+(-1)^i*(dli[%d]"
              ")^i;}\n",
              DIM - 1, i, i, i);
      dprintf(SF,
              "poly ChernD%d=ChernUp*ChernBottomD%d*ChernBottomHypersurface;\n",
              i, i);
      /*Chern classes of divisors*/
      for (j = 0; j < DIM - 1; j++)
        dprintf(SF, "poly c%dD%d=jet(ChernD%d,%d)-jet(ChernD%d,%d);\n", j + 1,
                i, i, j + 1, i, j);
    ...
    if (DIM == 3) {
      ...
      dprintf(SF, "list dP; list N; int l=0; int p=0; int t=0; int s=0; int "
                  "k=0; int u=0; int v=0;");
      dprintf(SF, ""
                  "for(i=1;i<=size(dli);i++){"
                  "if(reduce(dli[i],std(hypideal))!=0){"
                  "if(eulerHD[i]==1 && eulerD[i]>=3 && eulerD[i]<=11 && "
                  "reduce(c1D[i]^2*dli[i]*HySurf,chow)/norm >=1 && "
                  "reduce(c1D[i]^2*dli[i]*HySurf,chow)/norm <= 9){"
      ...
      dprintf(SF, "string "
                  "DelPezzo;for(i=1;i<=size(dP);i++){DelPezzo=DelPezzo+string("
                  "dP[i])+\"(\"+string(N[i])+\")\"+\" \";};");
      dprintf(SF, "list nonintDP; int g=0; int f=0; "
                  "if(size(dP)>0){if(size(dP)>1){for(i=1;i<=size(dP);i++){for("
                  "j=1;j<=size(dP);j++){if(dP[j]!=dP[i]){for(k=1;k<=size(dli);"
                  "k++){if(reduce(dP[i]*dP[j]*dli[k]*HySurf,chow)/"
                  "norm!=0){g++;} ;};};}; if(g==0){f++; nonintDP[f]=dP[i];}; "
                  "g=0;};} else {nonintDP[1]=dP[1];};};");
      dprintf(SF, "string "
                  "niDP;for(i=1;i<=size(nonintDP);i++){niDP=niDP+string("
                  "nonintDP[i])+\" \";};");
      dprintf(SF, "\"dPs:\",size(dP),\";\",DelPezzo,\"nonint:\",size(nonintDP),"
                  "\";\",niDP;");
    }
    /*#############################################*/
  }
