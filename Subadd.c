...
void Init_New_List(NF_List *S) {
  S->ANB = (SAVE_INC + SL_Nmax) * CperR_MAX;
...
int InfoSize(int rd, int lists, FInfoList *FI) {
  return (rd / 128 + rd + 1 + 4 + 2 * FI->nV + lists) +
         sizeof(int) * (9 + lists);
}
...
void Read_Honest_Poly(FILE *F, FInfoList *FI, NF_List *L) {
  ...
  L->nSLP = 2 * L->SLN - L->slSM - L->slNM;
  L->PEN = L->PPEN = L->peNM = L->peSM = 0;
  L->NP = L->savedNP = L->nSLP + 2 * FI->nNF - FI->nSM - FI->nNM;
  L->RemNB = 0;
  ...
  for (v = L->d + 1; v <= FI->nVmax; v++)
    if (FI->nNUC[v])
      for (nu = 1; nu <= FI->NUCmax; nu++)
        if (FI->NFnum[v][nu]) {
          unsigned j;
          FI->NF[v][nu] = &FI->NFli[pos];
          tNF += FI->NFnum[v][nu];
          for (j = 0; j < FI->NFnum[v][nu]; j++)
            for (i = 0; i < nu; i++)
              FI->NFli[pos++] = fgetc(F);
        }
  assert(pos == FI->NB);
...
void TestMSbits(NF_List *S, PolyPointList *_P) {
  ...
  UPint axNF = 0, axNM = 0;
  for (i = 0; i < S->PEN; i++) {
    unsigned char *C = &S->NewNF[S->PE[i].c];
    int ms = C[2] % 4;
    peNF++;
  ...
  for (i = 0; i < S->PPEN; i++) {
    unsigned char *C = &S->NewNF[S->PPE[i].pe.c];
    int ms = C[2] % 4;
    peNF++;
    { /* int nv=*C,nuc=C[1]; Test_ucNF(&S->d,&nv,&nuc,&C[2]);*/
    }
  ...
  for (v = S->d + 1; v <= S->Aux.nVmax; v++)
    if (S->Aux.nNUC[v])
      for (nu = 1; nu <= S->Aux.NUCmax; nu++)
        if ((tc = S->Aux.NFnum[v][nu]))
          for (i = 0; i < tc; i++) {
            unsigned char *C = &S->Aux.NF[v][nu][nu * i];
            int ms = C[0] % 4;
            axNF++;
            if (ms == 0)
              (axSM)++;
...
void Write_Bin_File(FILE *F, NF_List *L) {
  ...
  Print_Expect(_AI);
  fflush(stdout);
  nVmax = max(L->Aux.nVmax, AI.nVmax);
  NUCmax = max(L->Aux.NUCmax, AI.NUCmax);
  for (i = L->d + 1; i <= nVmax; i++) {
    int v_li = 0;
    for (j = 1; j <= NUCmax; j++)
      if ((NFnum[i][j] = L->Aux.NFnum[i][j] + AI.NFnum[i][j]))
        v_li++;
    li += v_li;
    if (v_li)
      fi++;
  ...
  fputc(NUCmax, F);
  fputUI(li, F);
  fputUI(L->Aux.nNF + AI.nNF, F);
  fputUI(L->Aux.nSM + AI.nSM, F);
  fputUI(L->Aux.nNM + AI.nNM, F);
#ifdef TEST_Write
  printf("\nd=%d  nV=%d nVmax=%d NUCmax=%d #lists=%d  ", L->d, fi, nVmax,
         NUCmax, li);
  fflush(stdout);
  ...
  for (v = L->d + 1; v <= nVmax; v++)
    if (AI.nNUC[v] || L->Aux.nNUC[v])
      for (nu = 1; nu <= NUCmax; nu++)
        if ((tc = NFnum[v][nu])) {
          unsigned int k;
          pen += AI.NFnum[v][nu]; /* n -> i; lipos -> j */
          for (i = 0; i < (L->Aux.NFnum[v][nu]); i++) {
            if (j < pen)
              while (L->PE[j].n == i) {
                C = &L->NewNF[L->PE[j++].c + 2];
                for (k = 0; k < nu; k++)
                  fputc(C[k], F);
                tc--;
                tnb -= nu;
                   ...
                   int *PEpos, int *FIpos, NF_List *S)   /* -1 exists(SL) */
{
  ...
  *PEpos = 0;
  if (!S->PEN)
    return 0;
  if ((i = PEntComp(FIpos, nv, nuc, uc, &S->PE[n0 = 0],
                    S))) /* i = PE[0] - uc */
  {
    if (i > 0) {
      return 0;
  ...
  while (n1 > n0 + 1) {
    *PEpos = (n0 + n1) / 2;
    i = PEntComp(FIpos, nv, nuc, uc, &S->PE[*PEpos], S);
    if (i)
      if (i > 0)
        n1 = *PEpos;
      else
  ...
  *PEpos = n1;
  return 0; /* exists: return 1; else PEpos=where it would go */
}
void Insert_PPent_into_Pent(NF_List *S) {
  ...
  if (++Spos)
    while (Spos--) {
      while (S->PPE[Spos].n < Mpos)
        S->PE[--pos] = S->PE[--Mpos];
      S->PE[--pos] = S->PPE[Spos].pe;
    }
  S->PEN += S->PPEN;
  S->PPEN = 0; /* Test_NF_List(S); */
}
                    ...
                    NF_List *S) /* exists :: 1 */
{
  ...
  *Ppos = 0;
  ...
  while (n1 > n0 + 1) {
    *Ppos = (n0 + n1) / 2;
    i = PPEntComp(Lpos, Mpos, nv, nuc, uc, &S->PPE[*Ppos], S);
    if (i)
      if (i > 0)
        n1 = *Ppos;
      else
  ...
  *Ppos = n1;
  return 0; /* exists: return 1; else Ppos=where it would go */
}
int SearchFIList(int *v, int *nu, unsigned char *uc, int *pos, FInfoList *FI) {
  ...
  *pos = 0;
  ...
  if ((i = RIGHTminusLEFT(uc, &FI->NF[*v][*nu][n0 = 0], nu))) {
    ...
    return 1;
  if ((i = RIGHTminusLEFT(
           uc, &FI->NF[*v][*nu][(*nu) * (n1 = FI->NFnum[*v][*nu] - 1)], nu))) {
    if (i < 0) {
      *pos = n1 + 1;
      return 0;
    }
  ...
  while (n1 > n0 + 1) {
    *pos = (n0 + n1) / 2;
    i = RIGHTminusLEFT(uc, &FI->NF[*v][*nu][(*nu) * (*pos)], nu);
    if (i)
      if (i > 0)
        n1 = *pos;
      else
  ...
  *pos = n1;
  return 0; /* exists: return 1; else pos=where it would go */
}
...
int SearchSL_List(int *nv, int *nuc, unsigned char *uc, int *SLnum,
                  NF_List *S) {
  ...
  *SLnum = 0;
  ...
  while (n1 > n0 + 1) {
    *SLnum = (n0 + n1) / 2;
    i = SL_Comp(nv, nuc, uc, &S->SLp[*SLnum], S);
    if (i)
      if (i > 0)
        n1 = *SLnum;
      else
  ...
  *SLnum = n1;
  return 0; /* exists: return 1; else SLnum=where it would go */
}
void SL_List_Insert(int *nv, int *nuc, unsigned char *uc, int *SLnum,
                    NF_List *S) {
  int l = S->SLN++;
  unsigned char *C = &(S->NewNF[S->NewNB]);
  if (*nuc + 2 + S->NewNB > S->ANB) {
...
void SL_List_Remove(int *NV, int *nUC, /* unsigned char *UC, */
                    int *SLnum, NF_List *S) {
  int l = *SLnum;
  unsigned char *C = &S->NewNF[S->SLp[l]];
  S->RemNB += (2 + C[1]);
  if (C[2] % 4)
    S->slNM--;
  else
...
int ucNF_Sort_Add(int *NV, int *nUC, unsigned char *UC, NF_List *S) {
  ...
  if (SearchSL_List(NV, nUC, UC, &NEWpos, S)) {
    ...
    if (NewH) {
      if (MirTest(*UC, *c)) {
        if ((*c % 4) != 3)
          SL_List_Remove(NV, nUC, /*UC,*/ &NEWpos, S);
        else {
          (*c) -= (*UC % 4);
          S->slNM++;
          S->NP--;
...
void Print_Statistics(NF_List *_L) {
  ...
  char bni[2];
  int BNI = (_L->nNF > 2000000) ? 1000000 : 1000;
  strcpy(bni, (BNI == 1000) ? "k" : "M");
  printf("%dkR-%d %dMB %d%sIP %d%sNF-%dk ", (int)_L->NP / 1000, (int)_L->nSLP,
         (int)((_L->NewNB - _L->RemNB + _L->Aux.NB) / 1000000),
         (int)(_L->nIP / BNI), bni, (int)_L->nNF / BNI, bni,
         (int)_L->nSLNF / 1000);
  printf("%d%s%d v%dr%d f%dr%d %db%d", _L->Nmin, _L->hc ? "-" : "_", _L->Nmax,
         _L->VN, _L->V, _L->FN, _L->F, _L->Xdif, _L->Xnuc);
  ...
  if (CPUsec < 1000)
    printf(" %ds %du %dn", REALsec, CPUsec, NFsec);
  else if ((CPUsec) < 60000)
    printf(" %dm %du %dn", REALsec / 60, CPUsec / 60, NFsec / 60);
  else
    printf(" %dh %du %dn", REALsec / 3600, CPUsec / 3600, NFsec / 3600);
#else
  CPUsec = (CLOCK - _L->CLOCK) / CLOCKS_PER_SEC;
  if (CPUsec < 1000)
    printf(" %ds %du", REALsec, CPUsec);
  else if ((CPUsec) < 60000)
    printf(" %dm %du", REALsec / 60, CPUsec / 60);
  else
    printf(" %dh %du", REALsec / 3600, CPUsec / 3600);
#endif
  _L->Nmin = 1000;
  _L->Nmax = 0;
  puts("");
...
#endif
  if ((_E->ne) > _L->F)
    _L->F = _E->ne; /* check vertex numbers */
  if ((_V->nv) > _L->V)
    _L->V = _V->nv;
...
void Bmin2BaseUCn(int *d, int *v, int *bmin, int *base, int *nuc) {
  ...
  if (*bmin <= BL->base[*v][i]) {
    while (i > 0 && *bmin <= (BL->base[*v][i - 1]))
      --i;
  } else {
    do
      AuxNextGoodBase(v, &nx, BL);
    while (*bmin > (BL->base[*v][++i]));
  }
  *base = BL->base[*v][i];
  *nuc = BL->nuc[*v][i];
}
...
#endif
/* UNIT :: ms+=4 */
void AuxVnf2ucNF(Long nf[POLY_Dmax][VERT_Nmax], int *d, int *v, int *off,
                 int *base, int *nuc, int *ms, unsigned char *UC) {
  int i, j, one = (*ms > 3);
  UXLong X;
  X.n = 0;
  for (i = 0; i < *d; i++)
              ...
              Long NF[POLY_Dmax][VERT_Nmax], int *MS)      /* OUT */
{
  ...
  if (*nuc % 2)
    X.x[X.n++] = uc[*nuc - 1];
  for (i = 0; i < *nuc / 2; i++)
    X.x[i] = uc[2 * i] + UCM * uc[2 * i + 1];
  i = UNIT_OFF;
  BaseGetInt(&i, MS, &X);
  one = (*MS > 3);
#ifdef USE_UNIT_ENCODE
  if (one) {
    UNIT_NUCtoBase(d, v, nuc, &base);
    BaseGetInt(&base, &off, &X);
  } else
#endif
...
int RIGHTminusLEFT(unsigned char *ucL, unsigned char *ucR, int *nuc) {
  ...
  while (--i)
    if (ucL[i] != ucR[i])
      return (ucL[i] < ucR[i]) ? 1 : -1;
  if (((*ucL) / 4) != ((*ucR) / 4))
    return (((*ucL) / 4) < ((*ucR) / 4)) ? 1 : -1;
  return 0;
}
void VF_2_ucNF(PolyPointList *P, VertexNumList *V, EqList *E, /* IN */
               int *NV, int *nUC, unsigned char *UC)          /* OUT */
{
...
#ifdef USE_UNIT_ENCODE
    if (vone)
      UNIT_Bmin2BaseUCn(&P->n, &V->nv, &vbmin, &vb, &vnuc);
    else
#endif
...
#ifdef USE_UNIT_ENCODE
    if (fone)
      UNIT_Bmin2BaseUCn(&P->n, &E->ne, &fbmin, &fb, &fnuc);
    else
#endif
  ...
  *NV = (MS == 2) ? E->ne : V->nv;
  ...
  if (MS < 2)
    ...
    AuxVnf2ucNF(F_NF, &P->n, &E->ne, &fo, &fb, &fnuc, &MSone, UC);
...
#ifdef USE_UNIT_ENCODE
      if (MSone > 3)
        UNIT_NUCtoBase(&P->n, NV, nUC, &base);
      else
#endif
...
void Add_Polya_2_Polyi(char *polyi, char *polya, char *polyo) {
  ...
  unsigned char ucI[NUC_Nmax], ucA[NUC_Nmax], *ucSL, *uc;
  int SLp[SL_Nmax];
  int IslNF, IslSM, IslNM, vI, nuI, i, d;
  unsigned Ili;
  Along IslNB;
  int AslNF, AslSM, AslNM, vA = 0, nuA = 0, a, j;
  unsigned Ali;
  Along AslNB;
  int slNF = 0, slSM = 0, slNM = 0, slNB = 0, slNP = 0, Oli = 0, v, nu, AmI = 0,
      ms, tnb = 0;
  Init_FInfoList(&FIi);
  Init_FInfoList(&FIa);
  if (!*polyi || !*polyo) {
  ...
  FIo.NUCmax = max(FIi.NUCmax, FIa.NUCmax);
  tnb = 0;
