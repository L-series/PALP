...
void Add_Polya_2_DBi(char *dbi, char *polya, char *dbo) {
  ...
  char *Ifx,
      *Ifn = (char *)malloc(j + strlen(dbi) + File_Ext_NCmax), *Ofx,
  ...
  if (NULL == (FI = fopen(Ifn, "r"))) {
    printf("Cannot open %s", Ifn);
    exit(0);
  }
    ...
    if (FIo.nNUC[v]) {
      ...
      if (FIi.nNUC[v]) {
        if (!newout) {
          ...
          assert(!rename(Ofn, Ifn));
        }
        if (NULL == (FI = fopen(Ifn, "rb"))) {
          printf("Ifn %s failed", Ifn);
          exit(0);
        }
      }
      if (NULL == (FO = fopen(Ofn, "wb"))) {
        printf("Ofn %s failed", Ofn);
        exit(0);
      }
    ...
    if (FIi.nNUC[v]) /* FIo.NFnum[v][nu]?=0 */
      for (nu = 1; nu <= FIi.NUCmax; nu++)
        if (FIi.NFnum[v][nu]) {
          ...
          if (FIo.NFnum[v][nu]) {
            FIo.nVmax = max(FIo.nVmax, v);
            FIo.NUCmax = max(FIo.NUCmax, nu);
            FIo.nNUC[v]++;
            Oli++;
          }
        }
...
void Extract_from_Hodge_db(char *dbname, char *x_string, PolyPointList *_P) {
  ...
  if (H1 && H2) {
    ...
    HM_to = max(H1, H2);
    HM_from = max(0, HM_to - Hod_Dif_max);
  }
      ...
      if (nnf_dh[dh][h12]) {
        ...
        while ((c1 = fgetc(Fh)) != EOF) {
          ...
          if (is_poly) {
            ...
            sprintf(com, "M:%d %d N:%d %d H:%d,%d [%d]", mp, mv, np, nv,
                    true_H1, true_H2, 2 * (true_H1 - true_H2));
            Print_PPL(_P, com);
          }
          if (is_dual && MS) {
            VertexNumList VNL;
            ...
            sprintf(com, "M:%d %d N:%d %d H:%d,%d [%d]", np, nv, mp, mv,
                    true_H2, true_H1, 2 * (true_H2 - true_H1));
...
int Read_H_ucNF_from_DB(DataBase *DB, unsigned char *uc) /* p=next read pos */
{
...
#ifdef TEST
  {
    ...
    if (totNF == 0) {
      int i, j;
      for (i = 1; i <= DB->NUCmax; i++)
        vNF += DB->NFnum[DB->v][i];
      for (i = DB->v; i <= DB->nVmax; i++)
        for (j = 1; j <= DB->NUCmax; j++)
          xvNF += DB->NFnum[i][j];
      if (xvNF != DB->nNF) {
        printf("xvNF=%d  nNF=%d", xvNF, DB->nNF);
        exit(0);
      }
      xvNF = vNF;
    }
  ...
  if (rest == 0) /* search next v/nu */
  {
    ...
    if ((v < DB->v) || (nu < DB->nu)) {
      assert(totNF == nuNF);
      nuNF += DB->NFnum[DB->v][DB->nu];
    }
    if (v < DB->v) {
      if (vNF) {
        printf("vNF[%d]=%d was=%d v_new=%d\n", v, xvNF, vNF, DB->v);
        exit(0);
      }
      for (i = 1; i <= DB->NUCmax; i++)
        vNF += DB->NFnum[DB->v][i];
...
int Read_H_poly_from_DB(DataBase *DB, PolyPointList *P) {
  ...
  EqList E;
  if (0 == ms3) {
    if (0 == Read_H_ucNF_from_DB(DB, uc))
      return 0;
  } else
    --uc[0];
  UCnf2vNF(&DB->d, &DB->v, &DB->nu, uc, NF, &MS);
  P->n = DB->d;
  P->np = DB->v;
  for (i = 0; i < P->np; i++)
    for (j = 0; j < P->n; j++)
...
int Read_H_poly_from_DB_or_inFILE(DataBase *DB, PolyPointList *P) {
  if ((DB == NULL) || (inFILE == NULL)) {
    CWS cws;
    return Read_CWS_PP(&cws, P);
  } else
