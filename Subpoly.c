...
int IsNewEq(Equation *E, CEqList *_C, EqList *_F, int *_n) {
  int i = _C->ne;
  while (i--)
    if (!CompareEq(E, &_C->e[i], *_n))
      return 0;
  i = _F->ne;
  while (i--)
    if (!CompareEq(E, &_F->e[i], *_n))
      return 0;
  return 1;
}
...
void Add_INCI_To_List(INCI New_INCI, INCI *INCI_List, int *n_INCI) {
  int i;
  for (i = *n_INCI - 1; i >= 0; i--) {
    if (INCI_LE(New_INCI, INCI_List[i]))
      return;
    if (INCI_LE(INCI_List[i], New_INCI))
      INCI_List[i] = INCI_List[--(*n_INCI)];
  }
...
INCI List_Complete(int n, int n_polys, int n_facets, INCI *polys, INCI *facets);

INCI Poly_Complete(int n, int *n_polyfacets, INCI *poly, INCI *polyfacets) {
  int j, k, n_cd2_faces = 0;
  INCI cd2_faces[CD2F_Nmax];
  INCI X = INCI_0();
  for (j = 0; j < *n_polyfacets; j++)
    X = INCI_OR(X, polyfacets[j]);
  if (n == 1)
    return INCI_XOR(X, *poly);
  for (j = 0; j < *n_polyfacets; j++)
    for (k = j + 1; k < *n_polyfacets; k++)
      Add_INCI_To_List(INCI_AND(polyfacets[j], polyfacets[k]), cd2_faces,
                       &n_cd2_faces);
  return INCI_OR(
      INCI_XOR(X, *poly),
...
INCI List_Complete(int n, int n_polys, int n_facets, INCI *polys,
                   INCI *facets) {
  ...
  for (i = 0; i < _CEq->ne; i++)
    for (j = 0; j < i; j++) {
      ...
      if (INCI_abs(New_Face) < _P->n - 1)
        continue;
      for (k = 0; k < _CEq->ne; k++)
        if (INCI_LE(New_Face, CEq_INCI[k]))
          if ((k != i) && (k != j))
            break;
      if (k < _CEq->ne)
        continue;
      for (k = 0; k < _E->ne; k++)
        if (INCI_LE(New_Face, E_INCI[k]))
          break;
      if (k < _E->ne)
        continue;
...
int Aided_IP_Check(PolyPointList *_P, VertexNumList *_V, EqList *_E,
  ...
  Hole_Verts = INCI_0();
  for (i = n_irrel; i < _E->ne; i++)
    Hole_Verts = INCI_OR(Hole_Verts, E_INCI[i]);
  for (i = 0; i < n_old_v; i++)
    _V->v[i] = i;
  _V->nv = n_old_v;
  _E->ne = n_irrel;
  ...
  if (n_irrel == 1) {
    ...
    for (i = n_old_v + 1; i < _P->np; i++) {
      ...
      if (dist == maxdist)
        if (Vec_Greater_Than(_P->x[i], _P->x[newvert], _P->n)) {
          newvert = i;
          maxdist = dist;
        }
    }
    ...
    for (i = 0; i < n_Hole_Faces; i++)
      CEq_INCI[i] = INCI_PN(Hole_Faces[i], 0);
...
void Drop_and_Keep(PolyPointList *_P, VertexNumList *_V, EqList *_E,
  ...
  if (red_P == NULL) {
    puts("Unable to allocate space for red_P");
    exit(0);
  }
  if (new2old == NULL) {
    puts("Unable to allocate space for new2old");
    exit(0);
  }

  for (j = 0; j < _P->n; j++)
    drop_point[j] = _P->x[_V->v[drop_num]][j];
  ...
  for (i = 0; i < _E->ne; i++) {
    if (Eval_Eq_on_V(&(_E->e[i]), drop_point, _P->n)) {
      for (j = 0; j < _P->n; j++)
        new_E->e[n_irrel].a[j] = _E->e[i].a[j];
      new_E->e[n_irrel++].c = _E->e[i].c;
    } else {
      for (j = 0; j < _P->n; j++)
        new_E->e[_E->ne + n_irrel - i - 1].a[j] = _E->e[i].a[j];
  ...
  for (i = 0; i < _V->nv; i++)
    if (i != drop_num) {
      for (j = 0; j < _P->n; j++)
        red_P->x[red_P->np][j] = _P->x[_V->v[i]][j];
  ...
  if (IP) {
    ...
    new_V.nv = red_V.nv;
    for (i = 0; i < red_V.nv; i++)
      if ((new_V.v[i] = new2old[red_V.v[i]]) == _P->np - 1)
        new_V.v[i] = _V->v[drop_num];

    /* Drop the vertex _V->v[drop_num]: */
    _P->np--;
    for (j = 0; j < _P->n; j++)
      _P->x[_V->v[drop_num]][j] = _P->x[_P->np][j];
    j = kept(_P->np, _KL);
    if (j >= 0)
    ...
    if (!IP_Check(_P, &test_V, &test_E) || (test_V.nv != red_V.nv) ||
      ...
      fprintf(outFILE, "_V: ");
      for (i = 0; i < _V->nv; i++)
        fprintf(outFILE, "%d ", _V->v[i]);
      fprintf(outFILE, "\n");
      fprintf(outFILE, "_E:\n");
      for (i = 0; i < _E->ne; i++) {
        for (k = 0; k < _P->n; k++)
          fprintf(outFILE, "%d ", (int)_E->e[i].a[k]);
        fprintf(outFILE, " %d\n", (int)_E->e[i].c);
      }
      fprintf(outFILE, "\n");
      ...
      for (i = 0; i < _P->n; i++)
        fprintf(outFILE, "%d ", (int)drop_point[i]);
      fprintf(outFILE, "\n");
      fprintf(outFILE, "_KL: ");
      for (i = 0; i < _KL->nk; i++)
        fprintf(outFILE, "%d ", _KL->k[i]);
      fprintf(outFILE, "\n");

      fprintf(outFILE, "red_V: ");
      for (i = 0; i < red_V.nv; i++)
        fprintf(outFILE, "%d ", red_V.v[i]);
      fprintf(outFILE, "\n");
      fprintf(outFILE, "new_E:\n");
      for (i = 0; i < new_E->ne; i++) {
        for (k = 0; k < _P->n; k++)
          fprintf(outFILE, "%d ", (int)new_E->e[i].a[k]);
        fprintf(outFILE, " %d\n", new_E->e[i].c);
      }
      fprintf(outFILE, "\n");
      fprintf(outFILE, "n_irrel: %d\n", n_irrel);
      fprintf(outFILE, "new2old:");
      for (i = 0; i < red_P->np; i++)
        fprintf(outFILE, " %d", new2old[i]);
      fprintf(outFILE, "\n");

      fprintf(outFILE, "test_V: ");
      for (i = 0; i < test_V.nv; i++)
        fprintf(outFILE, "%d ", test_V.v[i]);
      fprintf(outFILE, "\n");
      fprintf(outFILE, "test_E:\n");
      for (i = 0; i < test_E.ne; i++) {
        for (k = 0; k < _P->n; k++)
          fprintf(outFILE, "%d ", (int)test_E.e[i].a[k]);
        fprintf(outFILE, " %d\n", (int)test_E.e[i].c);
      }
      fprintf(outFILE, "\n");
...
void Start_Make_All_Subpolys(PolyPointList *_P, NF_List *_NFL) {
  ...
  if (_NFL->Nmin == 0) {
    PairMat PM;
    Make_VEPM(_P, &V, &E, PM); /* complete poly if */
  ...
  if (_NFL->kf) {
    ...
    for (i = 0; i < KL.nk; i++) {
      for (j = 0; j < _P->n; j++)
        fprintf(outFILE, "%d ", (int)_P->x[KL.k[i]][j]);
      fprintf(outFILE, "\n");
    }
  }
...
void IREgcd(Long *vec_in, int *d, Long *vec_out) {
  ...
  Long perm_vec_in[POLY_Dmax], perm_vec_out[POLY_Dmax], abs_sum,
      max_abs_sum = 0;
  for (i = 1; i <= *d; i++)
    n_perm *= i;
  for (i = 0; i < n_perm; i++) {
    ...
    abs_sum = 0;
    for (j = 0; j < *d; j++)
      abs_sum += labs(perm_vec_out[j]);
    if ((i == 0) || (abs_sum < max_abs_sum)) {
      max_abs_sum = abs_sum;
      for (j = 0; j < *d; j++)
        vec_out[j] = perm_vec_out[inv_perm[j]];
    }
  }
}
...
void Reduce_Poly(PolyPointList *_P, EqList *_E, KeepList *_KL, NF_List *_NFL,
  ...
  for (i = 0; i < _P->np; i++) {
    Long dist = 0;
    for (j = 0; j < _P->n; j++)
      dist -= BE.a[j] * _P->x[i][j];
    if (!(dist % BE.c)) {
      for (j = 0; j < _P->n; j++)
        new_P->x[new_P->np][j] = _P->x[i][j];
      if (kept(i, _KL) >= 0)
        new_KL.k[new_KL.nk++] = new_P->np;
      new_P->np++;
    }
  }
...
void Make_All_Subpolys(PolyPointList *_P, EqList *_E, VertexNumList *_V,
  ...
  for (i = 0; i < _E->ne; i++)
    if (_E->e[i].c > 1) {
      int nkept = 0;
      for (j = 0; j < _KL->nk; j++)
        if (!Eval_Eq_on_V(&(_E->e[i]), _P->x[_KL->k[j]], _P->n))
          nkept++;
      if (nkept > maxnkept) {
        maxnkept = nkept;
        badfacet = i;
  ...
  if (!_NFL->rf)
    if (badfacet == -1) {   /* not recover mode && reflexive */
  ...
  if (_NFL->rf == _NFL->rd)
    _NFL->rf = 0;
  if (_NFL->rf)
    for (i = 0; i < _NFL->b[_NFL->rd]; i++)
      if ((kept(_V->v[i], &new_KL) < 0))
        new_KL.k[new_KL.nk++] = _V->v[i];
  _NFL->rd++;
  ...
  if ((_NFL->of == 0) || (_NFL->of == -3))
    if (badfacet != -1) { /* -oc recover[of=-3] */
...
int Line_Recomb(int i, int line, int v, int f, subl_int x[][VERT_Nmax]) {
  ...
  for (j = line; j < f; j++) {
    ...
    if (!x[i][i] || (x[j][i] % x[i][i])) {
      changes = 1;
      if ((g = LEgcd(x[i][i], x[j][i], &y11, &y12)) < 0) {
        y11 *= -1;
        y12 *= -1;
        g *= -1;
      }
...
void Col_Recomb(int i, int col, int v, int f, subl_int x[][VERT_Nmax],
  ...
  if ((g = LEgcd(x[i][i], x[i][col], &y11, &y12)) < 0) {
    y11 *= -1;
    y12 *= -1;
    g *= -1;
  }
...
void MakePolyOnSublat(NF_List *_L, subl_int x[VERT_Nmax][VERT_Nmax], int v,
  ...
  for (i = 0; i < VERT_Nmax; i++) {
    ...
    subl_int min_entry, min_col_max = 0, min_lin_gcd = 0, min_lin_max = 0,
                        rem_gcd = 0;
    ...
    for (lin = i; lin < f; lin++) {
      ...
      for (col = i; col < v; col++)
        if (x[lin][col]) {
          if (!lin_gcd)
            lin_gcd = SI_abs(x[lin][col]);
          else
            lin_gcd = NNgcd(lin_gcd, x[lin][col]);
          if (SI_abs(x[lin][col]) > lin_max)
            lin_max = SI_abs(x[lin][col]);
        }
      if (lin_max)
        if ((!min_lin_gcd) || (lin_gcd < min_lin_gcd) ||
    ...
    for (col = i; col < v; col++)
      if (x[chosen_lin][col])
        if (SI_abs(x[chosen_lin][col]) <= min_entry) {
          subl_int col_max = 0;
          for (lin = i; lin < f; lin++)
            if (SI_abs(x[lin][col]) > col_max)
    ...
    while (SI_abs(x[i][i]) > rem_gcd) {
      ...
      if (Line_Recomb(i, i + 1, v, f, x))
        continue;
      if (SI_abs(x[i][i]) <= rem_gcd)
        break;
      for (col = i + 1; col < v; col++) {
...
void DPircheck(CWS *_W, PolyPointList *_P) {
  ...
  for (i = 0; i < _PD->np; i++) {
    ...
    for (k = 0; k < DE.ne; k++) {
      ...
      for (j = 0; j < _P->n; j++)
        Pairing += DE.e[k].a[j] * _PD->x[i][j];
      if (Pairing < 0)
        Good = 0;
    }
    if (Good) {
      for (j = 0; j < _P->n; j++)
        _RDP->x[_RDP->np][j] = _PD->x[i][j];
      _RDP->np++;
    }
  }
  /*       Print_PPL(_RDP);   */
