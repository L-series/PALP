...
INCI INCI_PN(INCI x, Long y) {
  ...
  z.ui[0] = (x.ui[0] << 1) | (!y);
  for (i = 1; i < I_NUI; i++)
    z.ui[i] = (x.ui[i] << 1) | (x.ui[i - 1] >> (INT_Nbits - 1));
  return z;
}
INCI INCI_D2(INCI x) {
  INCI z;
...
INCI Eq_To_INCI(Equation *_Eq, PolyPointList *_P, VertexNumList *_V) {
  ...
  INCI X = INCI_0();
  for (j = 0; j < _V->nv; j++)
    X = INCI_PN(X, EVAL_EQ(_Eq, _P->x[_V->v[j]], _P->n));
  return X;
}
            ...
            if (INCI_LE(*y, x)) {
              if (INCI_EQ(x, *y)) {
                *v = INCI_OR(*v, u);
                ...
                int l = k;
                *y = x;
                _I->f[d][k] = u; /* x>y :: y=x;f= */
                while ((++l) < (*n))
                  if (INCI_LE(_I->v[d][l], x)) {
                    (*n)--;
                    if (l < (*n)) {
                      _I->v[d][l] = _I->v[d][*n];
                      _I->f[d][l] = _I->f[d][*n];
...
void Make_VEPM(PolyPointList *_P, VertexNumList *_V, EqList *_E, PairMat PM) {
  ...
  for (i = 0; i < _E->ne; i++)
    for (j = 0; j < _V->nv; j++)
      PM[i][j] = EVAL_EQ(&_E->e[i], _P->x[_V->v[j]], _P->n);
}

int Transpose_PM(PairMat PM, PairMat DPM, int nv, int ne) {
  int i, j;
  if ((nv > EQUA_Nmax) || (ne > VERT_Nmax))
    return 0;
  for (i = 0; i < ne; i++)
    for (j = 0; j < nv; j++)
      DPM[j][i] = PM[i][j];
...
int Span_Check(EqList *_F, EqList *H, int *n) {
  ...
  for (i = 0; i < H->ne; i++) {
    for (j = 0; j < _F->ne; j++)
      if (!CompareEq(&(H->e[i]), &(_F->e[j]), *n))
        break;
    if (j == _F->ne)
      return 0;
  } /* didn't find H[i] among the F's */
  return 1;
}
...
int IsGoodCEq(Equation *_E, PolyPointList *_P, VertexNumList *_V) {
  int i = _V->nv;
  Long s;
  while (!(s = EVAL_EQ(_E, _P->x[_V->v[--i]], _P->n)))
    ;
  if (s < 0) {
    int j = _P->n;
    while (j--)
  ...
  while (i)
    if (EVAL_EQ(_E, _P->x[_V->v[--i]], _P->n) < 0)
      return 0;
  return 1;
}
...
int New_Start_Vertex(Long *V0, Long *Ea, PolyPointList *P, int *v) /* P.x[v] */
{
  ...
  for (i = 0; i < P->n; i++)
    E.a[i] = Ea[i];
  E.c = 0;
  E.c = -EVAL_EQ(&E, V0, P->n);
  d = EVAL_EQ(&E, P->x[0], P->n);
  if (d > 0)
    dp = d;
  if (d < 0)
    dn = d;
  for (i = 1; i < P->np; i++) {
    d = EVAL_EQ(&E, P->x[i], P->n);
    if (d == 0)
      continue;
    if (d == dp)
...
int GLZ_Start_Simplex(PolyPointList *_P, VertexNumList *_V, CEqList *_C) {
  ...
  if (x < *d) {
    ...
    for (i = 0; i < *d; i++)
      E->a[i] = Z[i];
    E->c = -EVAL_EQ(E, X, _P->n);
    if (EVAL_EQ(E, _P->x[_V->v[*d]], _P->n) < 0) {
      for (i = 0; i < *d; i++)
        E->a[i] = -Z[i];
      E->c *= -1;
    }
    ...
    for (i = 0; i < *d; i++)
      E->a[i] = Z[i];
    E->c = -EVAL_EQ(E, X, _P->n);
    XX = EVAL_EQ(E, _P->x[_V->v[*d - 1]], _P->n);
    assert(XX);
    if (XX < 0) {
      for (i = 0; i < *d; i++)
        E->a[i] = -Z[i];
  ...
  for (x = 0; x < _C->ne; x++)
    for (i = 0; i <= *d; i++)
      assert((x == i) ==
             (0 != EVAL_EQ(&_C->e[x], _P->x[_V->v[*d - i]], _P->n)));
  return 0;
}
...
void Make_New_CEqs(PolyPointList *_P, VertexNumList *_V, CEqList *_C,
  ...
  /* Thread-local scratch space — avoids data races when called from
     multiple threads simultaneously.  These buffers are only used as
     temporaries inside this function. */
  static __thread CEqList Bad_C;
  static __thread INCI Bad_C_I[CEQ_Nmax];
#else
  ...
  printf("V=%d F=%d: Ceq=%d", _V->nv, _F->ne, _C->ne);
  fflush(stdout);
#endif

  Bad_C.ne = _C->ne = 0;
  for (i = 0; i < Old_C_ne; i++) {
    Long dist = EVAL_EQ(&_C->e[i], _P->x[_V->v[_V->nv - 1]], _P->n);
  ...
  for (i = 0; i < _F->ne; i++)
    F_I[i] = INCI_PN(F_I[i],
                     EVAL_EQ(&_F->e[i], _P->x[_V->v[_V->nv - 1]], _P->n));
  for (j = 0; j < _F->ne; j++)
    if (!INCI_M2(F_I[j]))
      for (i = 0; i < Bad_C.ne; i++) {
        ...
        { int _ok = IsGoodCEq(&(_C->e[_C->ne]), _P, _V); _C->ne++; assert(_ok); }
      }
...
#if (SHOW_NEW_CEq)
  {
    ...
    char sm[2] = {'s', 0};
    int Rs = (int)difftime(DATE2, DATE1);
    if (Rs > 999) {
      Rs /= 60;
      *sm = 'm';
    }
...
int Ref_Check(PolyPointList *_P, VertexNumList *_V, EqList *_F) {
  ...
  for (i = 0; i < CEq->ne; i++)
    CEq_I[i] = Eq_To_INCI(&(CEq->e[i]), _P, _V);
  _F->ne = 0;
...
void add_for_completion(Long *yDen, Long Den, EqList *_E, PolyPointList *_CP,
                        int *old_np) {
  int i, n = _CP->n;
  Long yold[POLY_Dmax];
...
void Complete_Poly(PairMat VPM, EqList *_E, int nv, PolyPointList *_CP) {
  ...
  for (i = 1; i < _E->ne; i++) {
    InsPoint = i;
    while (InsPoint && (MaxDist[i] < MaxDist[OrdFac[InsPoint - 1]]))
      InsPoint--;
    for (j = i; j > InsPoint; j--)
      OrdFac[j] = OrdFac[j - 1];
    OrdFac[InsPoint] = i;
  }
  ...
  for (k = 0; k < n - 1; k++) { /* sets k=n-1; important!   */
    ...
    for (i = 0; i < n; i++)
      yDen[i] -= _E->e[BasFac[k]].c * InvMat[i][k];
  }
  position[n - 1] = -_E->e[BasFac[n - 1]].c - 1;
  for (i = 0; i < n; i++)
    yDen[i] -= (_E->e[BasFac[k]].c + 1) * InvMat[i][n - 1];
  while (k >= 0) {
    position[k]++;
    for (i = 0; i < n; i++)
      yDen[i] += InvMat[i][k];
    ...
    for (k = n - 1; (k >= 0); k--) {
      if (position[k] != MaxDist[BasFac[k]] - _E->e[BasFac[k]].c)
        break;
      position[k] = -_E->e[BasFac[k]].c;
      for (i = 0; i < n; i++)
...
void Make_FaceIPs(PolyPointList *_P, VertexNumList *_V, EqList *_E,
  ...
  for (k = 0; k < _P->np; k++) {
    x = INCI_0();
    for (i = 0; i < _E->ne; i++)
      x = INCI_PN(x, EVAL_EQ(&(_E->e[i]), _P->x[k], _P->n));
    RaiseNip(x, _I, _P->n);
  }
  for (k = 0; k < _DP->np; k++) {
    x = INCI_0();
...
void PrintFaceIPs(PolyPointList *_P, FaceInfo *_I) {
  ...
  for (i = 0; i <= M; i++) {
    printf("ip[%d]:", i);
    for (j = 0; j < _I->nf[i]; j++)
      printf(" %ld", (long)_I->nip[i][j]);
    puts("");
  }
  for (i = 0; i <= M; i++) {
    printf("dip[%d]:", i);
    for (j = 0; j < _I->nf[i]; j++)
      printf(" %ld", (long)_I->dip[i][j]);
    puts("");
  }
}
...
void RC_Calc_BaHo(PolyPointList *_P, VertexNumList *_V, EqList *_E,
  ...
  if (_FI == NULL) {
    printf("RC_Calc_BaHo: Unable to allocate _FI\n");
    exit(0);
  }
  _BH->mp = _P->np;
  _BH->mv = _V->nv;
...
void Qadd_for_completion(Long *yDen, Long Den, int n, int *np, EqList *_E,
  ...
  for (i = 0; i < n; i++)
    y[i] = yDen[i] / Den;
  for (i = 0; i < _E->ne; i++)
    x = INCI_PN(x, EVAL_EQ(&(_E->e[i]), y, n));
  RaiseDip(x, _I, n, 1);
  (*np)++;
}
...
void lastline(Long *EyD, Long *yDen, Long Den, EqList *_E, int n, int *np,
  ...
  xmin = INCI_0();
  for (i = 0; i < _E->ne; i++)
    xmin = INCI_PN(xmin, EVAL_EQ(&(_E->e[i]), y, n));
  ...
  xmax = INCI_0();
  for (i = 0; i < _E->ne; i++)
    xmax = INCI_PN(xmax, EVAL_EQ(&(_E->e[i]), y, n));
...
void QComplete_Poly(PairMat VPM, EqList *_E, int nv, int n, int *np,
  ...
  while (k >= 0) {
    ...
    for (j = 0; j < _E->ne; j++)
      EyD[j] += IME[k][j];
    lastline(EyD, yDen, Den, _E, n, np, MaxDist[BasFac[n - 1]], IME, InvMat,
             _I);
    for (k = n - 2; (k >= 0); k--) {
      if (position[k] != MaxDist[BasFac[k]] - _E->e[BasFac[k]].c)
        break;
