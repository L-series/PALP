...
void LocalPrintUsage(char *c, char hc) {
  ...
  while (hc != 'e') {
    if (hc == 'g') {
      printf(
          "Usage:     %s  [options] [ascii-input-file [ascii-output-file]]\n",
          c);
      printf("Options:   -h          print this information\n           ");
      puts("-f or -     use as filter; otherwise parameters denote I/O files");
      puts("           -m*         various types of minimality checks (* ... "
           "lvra)");
      puts("           -p* NAME    specification of a binary I/O file (* ... "
           "ioas)");
      puts("           -d* NAME    specification of a binary I/O database (DB) "
           "(* ... ios)");
      puts(
          "           -r          recover: file=po-file.aux, use same pi-file");
      puts(
          "           -o[#]       original lattice [omit up to # points] only");
      puts("           -s*         subpolytopes on various sublattices (* ... "
           "vphmbq)");
      ...
      puts("           -H*         applications related to Hodge number DBs (* "
           "...cstfe)");
    } else if (hc == 'm') {
      printf(
          "`%s -m*' reads a list of combined weight systems as ascii-input\n",
          c);
      ...
      printf(
          ...
          c);
      puts("              information on the above properties and the `span "
           "property'.");
    } else if (hc == 'p') {
      puts("With `-p*' you specify a binary I/O file that encodes a sorted "
           "list");
      ...
      puts("-paNAME or -pa NAME     specifies a list that should be added to "
           "another ");
      puts("         list given as a binary file (with -pi) or as a database "
           "(with -di).");
      puts("-psNAME or -ps NAME     specifies a list that should be subtracted "
           "from");
      puts("         another list given as a binary file (with -pi) .");
      puts("With -pa and -ps you have to specify output via -po or -do.");
    } else if (hc == 'd') {
      puts(
          "With `-d*' you specify a binary I/O database that encodes a sorted");
      ...
      puts("-dsNAME or -ds NAME     specifies a list in database format that "
           "should");
      puts("         be subtracted from another list given as a binary file "
           "(with -pi),");
      ...
      puts("sublattices:");
      printf("`%s -sh [-di<input-db>]' finds Calabi-Yau hypersurfaces that are "
             "free",
             c);
      ...
      printf("`%s -sp [-di<input-db>]' same as `-sh' except that it is checked "
             "whether",
             c);
      puts("\n             all points of the dual span the N lattice.");
      printf(
          "`%s -sv [-di<input-db>] [-mr] -po<out-file>' serves to determine\n",
          c);
      printf(
          "             on which sublattices of the original lattice a given ");
      printf(
      ...
      printf("`%s -sm [-di<input-db>] [-mr] -po<out-file>' same as `-sv' but ",
             c);
      printf("now all\n             reflexive polytopes that have the same "
             "pairing");
      puts(" matrix between\n             vertices and equations are "
           "constructed");
      puts("In addition there are the somewhat experimental options -sb, -sq "
           "for dim=4.");
      ...
      printf("`%s -k* [-di<input-db>] -po<out-file>' gives you a list of the "
             "vertices \n",
             c);
      puts("   of the input polytope and asks which of them should be kept;");
      puts("   all reflexive subpolytopes containing the kept vertices are "
           "determined.");
    } else if (hc == 'c') {
      printf("`%s -c' (or `-C') checks the consistency of a binary file or "
             "database\n",
             c);
      ...
      printf("`%s -M[M]' looks for polytopes in a list specified by `-pi' or\n",
             c);
      puts("`-di' whose mirrors are not in the same list. The resulting "
           "`missing");
      puts("mirrors' are written in ascii format.");
    } else if (hc == 'a') {
      printf("`%s -a[2b] -po<out-file>' converts ascii-input of reflexive "
             "polytopes\n",
             c);
      puts("to binary file format.");
      puts(
          "If an input file or database is specified via `-pi' or `-di', only");
      puts(
          "the polytopes not in one of these lists are written to <out-file>.");
      puts("If an ascii output file is explicitly specified, weights "
           "corresponding");
      ...
      printf("`%s -b[2a]' converts binary input to a list of normal forms in\n",
             c);
      puts("ascii format. For file input (specified via `-pi') all normal "
           "forms of");
      puts("polytopes on original lattices are displayed, but for database "
           "input");
      puts("(`-di') the normal forms of the sublattice polytopes in the "
           "database");
      puts("are shown. If no sublattice polytopes are left, then all "
           "polytopes ");
      puts("in the database are displayed.");
      puts("For non-reflexive binary input, -B[2A] should be used for "
           "conversion");
      puts("to ascii format.");
    } else if (hc == 'H') {
      puts("Options of the type `-H*' are used for handling Hodge number "
           "databases");
      puts("and work only for polytopes of dimension four. In particular,");
      printf("`%s -Hc [-vf#] [-vt#] -di<DB> -do<Hodge-DB>' calculates the\n",
             c);
      puts(
          "   Hodge numbers of the polytopes in DB and creates a Hodge number");
      puts("   database whose files correspond to fixed vertex numbers and "
           "Euler");
      puts("   numbers. If -vf and/or -vt are specified, only the Hodge "
           "numbers");
      ...
      printf(
          ...
          c);
      puts(
          "   from -Hc to one consisting of files of fixed Hodge number pairs");
      puts("   (with the same name if -do is omitted),");
      printf("`%s  -Ht -di<Hodge-DB>'  tests a Hodge-DB for consistency and\n",
             c);
      printf(
          "`%s  -Hf -pi<Hodge-DB-file>' serves for testing a Hodge-DB-file.\n",
          c);
      ...
      puts("   Unless at least one of h11, h12, E is specified, the search "
           "will");
      ...
      puts("   Binary files and databases always encode ordered lists of "
           "normal");
      puts("forms of polytopes, and any such structure created by some "
           "application");
      printf("of %s may be used as input for some other application of %s.\n",
             c, c);
      puts("   A database consists of various files NAME.<extension> and is "
           "specified");
      puts("via `-d* NAME'. It contains one ascii file and several binary "
           "files.");
      puts("   Ascii input should always correspond to a list of polytopes "
           "given");
      ...
      puts("followed by (#lines) lines each of which has (#colums) integers "
           "such");
    ...
    puts("");
    puts("Type one of [m,p,d,r,o,s,c,M,a,b,H] for help on options,");
    printf("`g' for general help, `I' for general information on I/O or `e' to "
           "exit: ");
    scanf("%s", &hc);
    puts("");
  }
...
int main(int narg, char *fn[]) {
  int n = 0, FilterFlag = 0, oFlag = 0, cFlag = 0, rFlag = 0, abFlag = 0,
      kFlag = 0, vf = 2, vt = VERT_Nmax - 1;
  char Blank = 0, *dbin = &Blank, *dbsub = &Blank, *dbout = dbin,
       *x_string = &Blank, *polyi = dbin, *polya = dbin, *polys = dbin,
       *polyo = dbin, mFlag = 0, HFlag = 0, sFlag = 0;
  static CWS W;
  PolyPointList *_P;
  if (narg == 1) {
  ...
  if (_P == NULL) {
    puts("Unable to allocate space for _P");
    exit(0);
  }
  ...
  while (narg > ++n)
    if (fn[n][0] != '-')
      ...
      case 'H': {
        ...
        HFlag = fn[n][2];
        if (HFlag == 'e')
          x_string = (fn[n][3]) ? &fn[n][3] : fn[++n];
      } break;
      case 'p': {
        if (fn[n][2] == 'i')
          polyi = (fn[n][3]) ? &fn[n][3] : fn[++n];
        else if (fn[n][2] == 'a')
          polya = (fn[n][3]) ? &fn[n][3] : fn[++n];
        else if (fn[n][2] == 's')
          polys = (fn[n][3]) ? &fn[n][3] : fn[++n];
        else if (fn[n][2] == 'o')
      ...
      case 'd': {
        if (fn[n][2] == 'i')
          dbin = (fn[n][3]) ? &fn[n][3] : fn[++n];
        else if (fn[n][2] == 's')
          dbsub = (fn[n][3]) ? &fn[n][3] : fn[++n];
        else if (fn[n][2] == 'o')
      ...
      case 'o':
        ...
        if (fn[n][2])
          if (fn[n][2] == 'c') {
            ...
            if (!oFlag) {
              oFlag = -2;
              puts("break recursion at RPs");
            } else
              printf("rec-dep<=%d\n", oFlag);
          }
        else
          printf("original lattices only\n");
        break;
      case 'v': {
        if (fn[n][2] == 'f')
          vf = atoi((fn[n][3]) ? &fn[n][3] : fn[++n]);
        if (fn[n][2] == 't')
          vt = atoi((fn[n][3]) ? &fn[n][3] : fn[++n]);
      } break;
      case 'x':
        PrintExtOptions();
        break;
