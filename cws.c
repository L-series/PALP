...
void PrintCWSUsage(char *c) {
  ...
  char *opt_string[OSL] = {
      ...
      "(optional).",
      "         -m# L H   make transverse weight systems for #-dimensional "
      "polytopes",
      "                   (fast). The lowest and highest degrees L<=H are "
      "required.",
      "         -c#       make combined weight systems for #-dimensional "
      "polytopes.",
      ...
      "points",
      "                   and vertices of a dual pair of IP polytopes; an "
      "entry  ",
...
void insertat(WSaux *X, weights ww, int position) {
  ...
  for (i = X->wnum - 1; i >= position; i--)
    for (j = 0; j <= X->N; j++)
      X->wli[i + 1].n[j] = X->wli[i].n[j];
  for (j = 0; j <= X->N; j++)
    X->wli[position].n[j] = ww.n[j];
  X->wnum++;
}
...
void addweight(WSaux *X, weights wn) {
  ...
  if (X->wnum) {
    if ((i = weicomp(wn, X->wli[n0 = 0], &X->N))) {
      if (i > 0) {
        insertat(X, wn, 0);
        return;
      }
    ...
    for (i = 0; i < npoints; i++) {
      ...
      if (one[i] >= 0) {
        ...
        for (ii = 0; ii < npoints; ii++)
          if (ii - i) { /* subtract multiple of i'th
line from ii'th line */
            for (j = 0; j < npoints; j++)
              if (j != one[i])
                rm[ii][j] = rD(rm[ii][j], rP(rm[ii][one[i]], rm[i][j]));
    ...
    for (i = 0; New && (i < nboundwei); i++) {
      New = 0;
      for (j = 0; j < X->N; j++)
        if (rD(newboundwei[j], boundwei[i][j]).N)
          New = 1;
    }
    if (New) {
      for (j = 0; j < X->N; j++)
        boundwei[nboundwei][j] = newboundwei[j];
      nboundwei++;
    }
  }
  for (j = 0; j < X->N; j++) {
    rattws[j] = rI(0);
    for (i = 0; i < nboundwei; i++)
      rattws[j] = rS(rattws[j], boundwei[i][j]);
    rattws[j] = rQ(rattws[j], rI(max(nboundwei, 1)));
  }
  tws.n[X->N] = 1;
  for (j = 0; j < X->N; j++)
    tws.n[X->N] = lcm(tws.n[X->N], rattws[j].D);
  for (j = 0; j < X->N; j++)
    ...
    if (npoints < X->N) {
      for (x0 = 0; x0 * tws.n[0] < tws.n[X->N]; x0++)
        for (x1 = 0; x0 * tws.n[0] + x1 * tws.n[1] < tws.n[X->N]; x1++)
          for (x2 = 0;
               x0 * tws.n[0] + x1 * tws.n[1] + x2 * tws.n[2] < tws.n[X->N];
               x2++)
            /* #if (N>3) */ for (x3 = 0; (x3 == 0) ||
                                         ((X->N > 3) &&
                                          (x0 * tws.n[0] + x1 * tws.n[1] +
                                               x2 * tws.n[2] + x3 * tws.n[3] <
                                           tws.n[X->N]));
                                 x3++)
              /* #if (N>4) */ for (x4 = 0; (x4 == 0) ||
                                           ((X->N > 4) &&
                                            (x0 * tws.n[0] + x1 * tws.n[1] +
                                                 x2 * tws.n[2] + x3 * tws.n[3] +
                                                 x4 * tws.n[4] <
                ...
                sum = 0;
                maxx = 0;
                X->points[npoints][0] = x0;
                sum += x0;
                maxx = max(maxx, x0);
                X->points[npoints][1] = x1;
                sum += x1;
                maxx = max(maxx, x1);
                X->points[npoints][2] = x2;
                sum += x2;
                maxx = max(maxx, x2);
                if (X->N > 3) {
                  X->points[npoints][3] = x3;
                  sum += x3;
                  maxx = max(maxx, x3);
                }
                if (X->N > 4) {
                  X->points[npoints][4] = x4;
                  sum += x4;
                  maxx = max(maxx, x4);
                }
                if ((sum > 2) && (maxx > 1))
...
void makesubsets(WSaux *X) {
  ...
  for (i = 0; i < X->N - 1; i++)
    X->nsubsets[i] = 0;
  for (p0 = 0; p0 < X->N - 1; p0++)
    for (p1 = p0 + 1; p1 < X->N; p1++) {
...
void RecMoonWeights(Weight *W, int g, int sum, long *npp, long *nintPP1,
  ...
  if (n)
    ...
    if (PP1N[0] % PP1D[0])
      return;
    if (PP1N[n] / PP1N[n + 1] != W->d - W->w[n])
      return;
    (*nintPP1)++;
#if (MOONSHINE_CRITERIA)
    dmwow[n] = rR(W->d - W->w[n], W->w[n]);
    if (IfMoonWWrite(W, dmwow, nsmallchi, stf))
      (*nintchi)++;
#else
    if (Trans_Check(*W)) {
      WRITE_Weight(W);
...
#endif
      RecMoonWeights(&W, Fgcd(W.d, W.w[W.N - 1]), W.d - W.w[W.N - 1], &npp,
...
void Make_Trans_Weights(int n, int dmin, int dmax /*,int rFlag */) {
  ...
  for (X.d = dmin; X.d <= dmax; X.d += inc) {
    ...
    if (ALLOWHALF)
      ...
      X.jmax = (X.d - 1) / 2;
    if (CHAT)
      T_Chon(1, -1, (X.d * (n - CHAT)) / 2 - n + 1, X.d, &X);
    else
...
void T_Chon(int i, int urp, int nm, int g, T_aux *X) {
  ...
  if (i < X->n)
    for (j = (i == X->n - 1) ? (1 + nm - jm) : 1; j <= jm; j++) {
      ...
      if (urp < 0) {
        if (X->d % j) {
          res = 1; /* i.e. not ferm; res=0 -> resolved */
          for (l = 1; (l < i) && res; l++)
            res = (X->d - X->wei[l]) % j;
          if (res)
            T_Chon(ip, i, nm - j + 1, Fgcd(g, j), X);
          else
            T_Chon(ip, 0, nm - j + 1, Fgcd(g, j), X);
        } else {
          if ((i == 1) || (j >= X->wei[i - 1]))
...
void T_Addweight(T_weight win, T_aux *X) {
  ...
  for (i = 1; i < wn[0]; i++)
    for (p = i + 1; p <= wn[0]; p++)
      if (wn[i] > wn[p])
        swap(&wn[i], &wn[p]); /* make n0<=n1<=...<=n# */
  if (X->wnum) {
    if ((i = T_Weicomp(wn, X->wli[n0 = 0]))) {
      if (i > 0) {
        T_Insertat(wn, 0, X);
        return;
      }
...
const wei4 W4[95] = {
    ...
    {10, {1, 2, 3, 4}},   {10, {1, 2, 2, 5}},   {10, {1, 1, 3, 5}},
    {11, {1, 2, 3, 5}},   {12, {2, 3, 3, 4}},   {12, {1, 3, 4, 4}},
    {12, {2, 2, 3, 5}},   {12, {1, 2, 4, 5}},   {12, {1, 2, 3, 6}},
    {12, {1, 1, 4, 6}},   {13, {1, 3, 4, 5}},   {14, {2, 3, 4, 5}},
    {14, {2, 2, 3, 7}},   {14, {1, 2, 4, 7}},   {15, {3, 3, 4, 5}},
    {15, {2, 3, 5, 5}},   {15, {1, 3, 5, 6}},   {15, {1, 3, 4, 7}},
    {15, {1, 2, 5, 7}},   {16, {1, 4, 5, 6}},   {16, {2, 3, 4, 7}},
    {16, {1, 3, 4, 8}},   {16, {1, 2, 5, 8}},   {17, {2, 3, 5, 7}},
    {18, {3, 4, 5, 6}},   {18, {1, 4, 6, 7}},   {18, {2, 3, 5, 8}},
    {18, {2, 3, 4, 9}},   {18, {1, 3, 5, 9}},   {18, {1, 2, 6, 9}},
    {19, {3, 4, 5, 7}},   {20, {2, 5, 6, 7}},   {20, {3, 4, 5, 8}},
    {20, {2, 4, 5, 9}},   {20, {2, 3, 5, 10}},  {20, {1, 4, 5, 10}},
    {21, {3, 5, 6, 7}},   {21, {1, 5, 7, 8}},   {21, {2, 3, 7, 9}},
    {21, {1, 3, 7, 10}},  {22, {2, 4, 5, 11}},  {22, {1, 4, 6, 11}},
    {22, {1, 3, 7, 11}},  {24, {3, 6, 7, 8}},   {24, {4, 5, 6, 9}},
    {24, {1, 6, 8, 9}},   {24, {3, 4, 7, 10}},  {24, {2, 3, 8, 11}},
    {24, {3, 4, 5, 12}},  {24, {2, 3, 7, 12}},  {24, {1, 3, 8, 12}},
    {25, {4, 5, 7, 9}},   {26, {2, 5, 6, 13}},  {26, {1, 5, 7, 13}},
    {26, {2, 3, 8, 13}},  {27, {5, 6, 7, 9}},   {27, {2, 5, 9, 11}},
    {28, {4, 6, 7, 11}},  {28, {3, 4, 7, 14}},  {28, {1, 4, 9, 14}},
    {30, {5, 6, 8, 11}},  {30, {3, 4, 10, 13}}, {30, {4, 5, 6, 15}},
    {30, {2, 6, 7, 15}},  {30, {1, 6, 8, 15}},  {30, {2, 3, 10, 15}},
    {30, {1, 4, 10, 15}}, {32, {4, 5, 7, 16}},  {32, {2, 5, 9, 16}},
    {33, {3, 5, 11, 14}}, {34, {4, 6, 7, 17}},  {34, {3, 4, 10, 17}},
    {36, {7, 8, 9, 12}},  {36, {3, 4, 11, 18}}, {36, {1, 5, 12, 18}},
    {38, {5, 6, 8, 19}},  {38, {3, 5, 11, 19}}, {40, {5, 7, 8, 20}},
    {42, {3, 4, 14, 21}}, {42, {2, 5, 14, 21}}, {42, {1, 6, 14, 21}},
...
void STtmp(FILE *w2FILE, FILE *w3FILE, FILE *w4FILE) {
  ...
  for (i = 0; i < 95; i++) {
    fprintf(w4FILE, "%d  ", W4[i].d);
    for (j = 0; j < 4; j++)
      fprintf(w4FILE, "%d ", W4[i].w[j]);
    fprintf(w4FILE, "\n");
  }
  for (i = 0; i < 3; i++) {
    fprintf(w3FILE, "%d  ", W3[i].d);
    for (j = 0; j < 3; j++)
      fprintf(w3FILE, "%d ", W3[i].w[j]);
    fprintf(w3FILE, "\n");
  }
  fprintf(w2FILE, "%d  ", W2.d);
  for (j = 0; j < 2; j++)
    fprintf(w2FILE, "%d ", W2.w[j]);
  fprintf(w2FILE, "\n");
  rewind(w2FILE);
  rewind(w3FILE);
  rewind(w4FILE);
...
void Make_34_CWS(int d) {
  ...
  if (d == 3) {
    ...
    u = 1;
    ef = 1;
    mkold2(outfile, w3FILE, w3FILE, u, ef);
    /*outfile="2x3"*/;
    u = 0;
    ef = 0;
    mkold2(outfile, w2FILE, w3FILE, u, ef);
    /*outfile="2x2x2"*/;
    mk2xxx(outfile, atoi("3"));
  }
  if (d == 4) {
    outfile = "";
    /*outfile="4uu4";*/ u = 2;
    ef = 1;
    mkold2(outfile, w4FILE, w4FILE, u, ef);
    /*outfile="3u4"; */ u = 1;
    ef = 0;
    mkold2(outfile, w3FILE, w4FILE, u, ef);
    /*outfile="3x3"; */ u = 0;
    ef = 1;
    mkold2(outfile, w3FILE, w3FILE, u, ef);
    /*outfile="2x4"; */ u = 0;
    ef = 0;
    mkold2(outfile, w2FILE, w4FILE, u, ef);
    /*outfile="3u3x2";*/ u = 1;
    ef = 1;
    mkold_nno(outfile, w3FILE, w3FILE, w2FILE, u, ef);
    /*outfile="3x2x2";*/ u = 0;
    ef = 0;
    mkold_nno(outfile, w3FILE, w2FILE, w2FILE, u, ef);
...
void W_TO_CWS(
    CWS *CW, Weight *_W, int Nf, int Nr, int Nb,
    int u) { /* d (Nf x 0) w_1 ... w_u  (Nb x 0)  w_(u+1) ... w_N  (Nr x 0) */
  int i, j, d = 0;
  CW->d[CW->nw] = _W->d;
  CW->N = (Nf + Nb + Nr + _W->N);
  if (CW->N > AMBI_Dmax)
    Die("increase AMBI_Dmax !");
  for (i = 0; i < Nf; i++)
    CW->W[CW->nw][i] = 0;
  for (i = 0; i < _W->N; i++) {
...
void RW_TO_CWS(
    CWS *CW, Weight *_W, int Nf, int Nr, int Nb,
    int u) { /* d (Nf x 0) w_N ... w_(u+1)  (Nb x 0)  w_u ... w_1  (Nr x 0) */
  int i, j, d = 0;
  CW->d[CW->nw] = _W->d;
  CW->N = (Nf + Nb + Nr + _W->N);
  if (CW->N > AMBI_Dmax)
...
void Make_IP_CWS(int narg, char *fn[]) {
  ...
  while (narg > ++n) {
    ...
    if (fn[n][1] == 'c') {
      if ((fn[n][2] == 0) && (narg > n + 1))
        ...
        a = &fn[n][2];
      if (!IsDigit(*a))
        Die("after -c there must be a digit!");
      d = atoi(a);
    }
    if (fn[n][1] == 'n') {
      if ((fn[n][2] == 0) && (narg > n + 1))
        ...
        a = &fn[n][2];
      if (!IsDigit(*a))
        Die("after -n there must be a digit!");
      nF = atoi(a);
      n++;
      break;
    }
  ...
  if (nF == 3) {
    ...
    if (((t.u[0] == 1) && (t.u[1] == 1) && (t.u[2] == 1)) ||
      ...
      eq[0] = 0;
      eq[1] = 0;
      if (!strcmp(infile[0], infile[1]))
        eq[0] = 1;
      if (!strcmp(infile[1], infile[2]))
        eq[1] = 1;
...
int ConvHull(PolyPointList *P1, PolyPointList *P2, PolyPointList *P,
  ...
  if ((P1->n + x) > POLY_Dmax)
    Die("increase POLY_Dmax!");
  if (x < 0)
    Die("if input is -p#, # must be less than dim of first poly");

  P->np = V1->nv;
  for (i = 0; i < V1->nv; i++) {
    for (j = P1->n; j < (P2->n + x); j++)
      P->x[i][j] = 0;
    for (j = 0; j < P1->n; j++)
