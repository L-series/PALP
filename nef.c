...
void ChangeToTrianBasis(AmbiPointList *, AmbiLatticeBasis *, PolyPointList *);
...
void PrintNefUsage(char *c) {
  ...
  char *opt_string[OSL] = {
      ...
      "         -y        prints poly/CWS in M lattice if it has "
      "nef-partitions",
      ...
      "             -uPOINTS  ... upper limit of #points (default = "
      "POINT_Nmax)",
      "             -lPOINTS  ... lower limit of #points (default = 0)",
      "         -m        starts with [d  w1 w2 ... wk d=d_1 d_2 (Minkowski "
      "sum)",
...
int main(int narg, char *fn[]) {
  ...
  if (_PS == NULL)
    Die("Unable to alloc space for Pstat _PS");
...
void Make_Sort(Long *_N, Long *_n, Long *_B_num, Long *_c_num, Long *_c_less) {
  Long N, n;
...
void Mink_WPCICY(AmbiPointList *_AP_1, AmbiPointList *_AP_2,
                 AmbiPointList *_AP) {
  int l;
  Long *_x, *_B_num, *_c_num, *_c_less, n = 0, N = 0, Num, num, P_max, p_max, j,
                                        k, m;
  DYN_PPL B;
