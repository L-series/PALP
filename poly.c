...
int main(int narg, char *fn[]) {
  int n = 0, k, FilterFlag = 0, lg = 0, s = 0, i = 0, m = 0, p = 0, v = 0,
      ...
      D = 0, IP, R, Tr, T = 0, PS = 0, VS = 0, CD = 0, ZS = 1, A = 0, B = 0,
      G = 0, F = 0, U = 0, dd = 0, Einstein = 0, o = 0, q = 0, Q = 0;
  char c;
  ...
  if ((CW == NULL) || (E == NULL) || (_P == NULL) || (DE == NULL) ||
      (_DP == NULL) || (PM == NULL) || (DPM == NULL)) {
  ...
  if (s + i + I + m + p + v + e + d + t + S + N + T + PS + VS + CD + G + A + B +
          F + z ==
      0)
    g = 1;
  VH.sts = (lg == 2);
  if ((U == 1) && (CD == 1) &&
      (s + i + I + m + p + v + e + d + t + S + N + T + PS + VS + G + A + B + F +
           z ==
       0))
    g = 1;
  if (g + lg + p + d + PS + CD == 0)
    nc = 1; /* don't need completion of points */
  if ((T == 1) && (B + U + lg + g + s + i + I + m + p + v + e + d + t + S + N +
                       PS + VS + (1 - ZS) ==
                   0)) {
  ...
  if (i) {
    ...
    if (FI == NULL) {
      puts("Unable to allocate space for FaceInfo FI");
      exit(0);
    }
  }
  if (Q)
  ...
  while (lg ? Read_W_PP(&W, _P) : Read_CWS_PP(CW, _P)) {
    ...
    if (T) {
      if (CW->nw) {
        puts("Please do not use weight input with the -T option");
        continue;
      } else
    ...
    if (U == 2) {
      VNL_to_DEL(_P, &V, DE);
      if (!ConifoldSing(_P, &V, E, _DP, DE, dd))
        continue;
    }
    if (g) {
      if (!R || nc) {
    ...
    if (v)
      Print_VL(_P, &V, "Vertices of P");
    if (e)
      Print_EL(
          E, &_P->n, R,
          (R ? "Vertices of P-dual <-> Equations of P" : "Equations of P"));
    if (i) {
      Make_Incidence(_P, &V, E, FI);
    ...
    if (R && (PS || VS || CD))
      IP_Simplices(_DP, (!D) * E->ne, PS * ZS, VS * ZS, CD);
    if (G) {
      char divi[99];
      Long g = Divisibility_Index(_P, &V);
      if (g > 1) {
